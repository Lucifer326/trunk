...
/**
 * 通过hibernate和脚本来处理同步数据库 对于修改数据库的字段，考虑各种数据库的情况，字段名称全部、类型修改成大写
 */
public class DbTableProcess {
	 ...
	 * @throws DBException
	 */
	public static void createOrDropTable(CgFormHeadEntity table, Session session)
			throws IOException, TemplateException, HibernateException, SQLException, DBException {
		Template t;
	...
	public List<String> updateTable(CgFormHeadEntity table, Session session) throws DBException {
		...
		String tableName = DbTableUtil.getDataType(session).equals("ORACLE") ? table.getTableName().toUpperCase()
				: table.getTableName();
		...
		try {
			...
			// 表如果不存在该列，则要对表做修改、增加、删除该列动作
			// 此处无法处理删除的列，因为在这个循环中无法获得该列
	...
	private static void createTable(String xml, CgFormHeadEntity table, Session session)
		...
		// update-begin--Author:Robin Date:20140507 for：TASK #409
		// 字段长度未注明时，显示同步数据库成功，但实则未创建数据表
		// 抛出执行异常，抛出第一个即可
		...
		// update-end--Author:Robin Date:20140507 for：TASK #409
		// 字段长度未注明时，显示同步数据库成功，但实则未创建数据表
	}
	 ...
	 * @throws SQLException
	 */
	public static Map<String, ColumnMeta> getColumnMetadataFormDataBase(String schemaName, String tableName,
		...
		ResultSet rs = dbMetaData.getColumns(null, schemaName, tableName.toUpperCase(), "%");

		ColumnMeta columnMeta;

		Map<String, ColumnMeta> columnMap = new HashMap<String, ColumnMeta>();

		logger.info("conn:" + conn + "dbMetaData:" + dbMetaData + "rs:" + rs + "columnMap:" + columnMap.size());
		...
		while (rs.next()) {
			...
			logger.info("getColumnMetadataFormDataBase --->COLUMN_NAME:"+columnName+" TYPE_NAME :"+typeName
					+" DECIMAL_DIGITS:"+decimalDigits);
			columnMap.put(columnName, columnMeta);
			
			logger.info("获取数据库中列的描述---------->COLUMN_NAME:" + rs.getString("COLUMN_NAME") + " TYPE_NAME :"
					+ rs.getString("TYPE_NAME") + " DECIMAL_DIGITS:" + rs.getInt("DECIMAL_DIGITS") + " COLUMN_SIZE:"
					+ rs.getInt("COLUMN_SIZE") + "Comment:" + rs.getString("REMARKS"));
			columnMap.put(rs.getString("COLUMN_NAME").toLowerCase(), columnMeta);
		}
		logger.info("columnMap的大小值:" + columnMap.size());
		return columnMap;
	}
	...
	public static Map<String, ColumnMeta> getColumnMetadataFormCgForm(CgFormHeadEntity table) {
		...
		for (CgFormFieldEntity cgFormFieldEntity : cgFormFieldEntities) {
			...
			columnMeta.setOldColumnName(cgFormFieldEntity.getOldFieldName() != null
					? cgFormFieldEntity.getOldFieldName().toLowerCase() : null);

			logger.info("getColumnMetadataFormCgForm ---->COLUMN_NAME:" + cgFormFieldEntity.getFieldName().toLowerCase()
					+ " TYPE_NAME:" + cgFormFieldEntity.getType().toLowerCase() + " DECIMAL_DIGITS:"
					+ cgFormFieldEntity.getPointLength() + " Comment:" + cgFormFieldEntity.getContent());
	...
	private int updateFieldName(String columnName, String id, Session session) {
		return session
				.createSQLQuery("update cgform_field set old_field_name= '" + columnName + "' where id='" + id + "'")
				.executeUpdate();
	}
