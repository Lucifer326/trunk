        ...
        <div class="row">
            <form role="form" id="myForm" class="form-horizontal m-t">
                <div class="form-group draggable">
                    <input type="hidden" name="del" value="${productEditSel.del}">
                    <input type="hidden" name="uuid" value="${productEditSel.uuid}">
                    <input type="hidden" name="orderNum" value="${productEditSel.createName}">
                    <input type="hidden" name="orderNum" value="${productEditSel.createBy}">
                    <input type="hidden" name="orderNum" value="${productEditSel.createDate}">
                    <label class="col-sm-2 control-label"><span class="start">*</span>公司名称：</label>
                    <div class="col-sm-10">
                        <input type="text" name="name" class="form-control" id="name" value="${productEditSel.name}" onkeyup="this.value=this.value.replace(/[`~!@·%#$^&*()=|{}':;',\[\].<>/?~！@#￥……&*（）——|{}【】‘；：”“'。，、？ ]/g,'');">
                    </div>
                </div>
                <div class="form-group draggable">
                    <label class="col-sm-2 control-label">公司简介：</label>
                    <div class="col-sm-10">
                        <textarea class="form-control" rows="3" name="companyInfo" id="companyInfo">${productEditSel.companyInfo}</textarea>
                    </div>
                </div>
                <div class="form-group draggable">
                    <label class="col-sm-2 control-label">是否参加过政府招标：</label>
                    <div class="col-sm-10">
                        <c:forEach items="${editIsNot}" var="item">
                            <c:if test="${productEditSel.tender eq item.typecode}">
                                <div class="radio radio-inline radio-danger">
                                    <input type="radio" id="radio15" value="${item.typecode}" name="tender" checked>
                                    <label for="radio1">${item.typename} </label>
                                </div>
                            </c:if>
                            <c:if test="${ productEditSel.tender != item.typecode}">
                                <div class="radio radio-inline radio-danger">
              ...
              <%--          <div class="radio radio-inline radio-danger">
                                <input type="radio" id="radio15" value="Y" name="tender" ${productEditSel.tender=='Y'?'checked':''}>
                            <label for="radio1">是 </label>
                        </div>
                        <div class="radio radio-inline radio-danger">
                                <input type="radio" id="radio16" value="N" name="tender" ${productEditSel.tender=='N'?'checked':''}>
				...
				<div class="form-group draggable">
                    ...
                    <div class="col-sm-10">
                        <c:forEach items="${editIsNot}" var="item">
                            <c:if test="${productEditSel.inPut eq item.typecode}">
                                <div class="radio radio-inline radio-danger">
                                    <input type="radio" id="radio15" value="${item.typecode}" name="inPut" checked>
                                    <label for="radio1">${item.typename} </label>
                                </div>
                            </c:if>
                            <c:if test="${ productEditSel.inPut != item.typecode}">
                                <div class="radio radio-inline radio-danger">
                ...
                <div class="form-group draggable">
                    ...
                    <div class="col-sm-10">
                        <input type="text" name="product" class="form-control" id="product" value="${productEditSel.product}" onkeyup="this.value=this.value.replace(/[`~!@·%#$^&*=|{}\[\]?~！@#￥……&*（）——|{}【】？ ]/g,'');">
                    </div>
                </div>
                <div class="form-group draggable">
                    <label class="col-sm-2 control-label"><span class="start">*</span>联系人：</label>
                    <div class="col-sm-4">
                        <input type="text" name="contact" class="form-control" value="${productEditSel.contact}" id="contact">
                    </div>
                    <label class="col-sm-2 control-label"><span class="start">*</span>联系电话：</label>
                    <div class="col-sm-4">
                        <input type="text" name="phone" id="phone" class="form-control" value="${productEditSel.phone}">
                    </div>
                </div>
                <div class="form-group draggable">
                    <label class="col-sm-2 control-label">邮箱：</label>
                    <div class="col-sm-10">
                        <input type="text" name="email" id="email" class="form-control" value="${productEditSel.email}">
                    </div>
                </div>
                <div class="form-group draggable">
                    <label class="col-sm-2 control-label">网址：</label>
                    <div class="col-sm-10">
                        <input type="text" name="url" id="url" class="form-control" value="${productEditSel.url}">
                    </div>
                </div>
                <div class="form-group draggable">
                    <label class="col-sm-2 control-label">地址：</label>
                    <div class="col-sm-10">
                        <input type="text" name="address" class="form-control" id="address" value="${productEditSel.address}" onkeyup="this.value=this.value.replace(/[`~!@·%#$^&*()=|{}':;',\[\].<>/?~！@#￥……&*（）——|{}【】‘；：”“'。，、？ ]/g,'');">
                    </div>
                </div>
                ...
                <div class="form-group draggable">
                    ...
                    <div class="col-sm-10">
                        <textarea class="form-control" rows="3" name="remark" id="remark">${productEditSel.remark}</textarea>
                    </div>
                </div>
            </form>
        </div>
