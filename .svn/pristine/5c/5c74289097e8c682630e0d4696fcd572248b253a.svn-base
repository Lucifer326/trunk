...
public class IconController extends BaseController {
	...
	public void datagrid(TSIcon icon,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		...
		if (StringUtil.isNotEmpty(iconName)) {
			
			cq.like("iconName", "%" + iconName + "%");
			
		}
		if (StringUtil.isNotEmpty(iconType)) {
			
			cq.eq("iconType", Short.parseShort(iconType));
			
		}
		cq.add();
		this.systemService.getDataGridReturn(cq, true);

		//update-begin--  Date:20180210 for：修改图标不能正确回显问题--------------------
		//IconImageUtil.convertDataGrid(dataGrid, request);//先把数据库的byte存成图片到临时目录，再给每个TsIcon设置目录路径
		//update-end--  Date:20180210 for：修改图标不能正确回显问题--------------------
