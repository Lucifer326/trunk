...
//--------longjb-end--20150526 ----for:add step.3 判断是否有附件字段,进行连带删除附件及附件表---------------
	...
	//Online表单在线开发时，创建表单，点击提交流程   获取processkey
			public TPProcess getProcesskeyConfigId(String id){
				
				String hql = "select p from TPProcess p, TSBusConfig s where p.id = s.TPProcess.id and s.onlineId = ? ";
				
				List<TPProcess> list = commonDao.findHql(hql, id);
