...
import org.jeecgframework.web.system.pojo.base.TSRoleFunctionBase;
...
public class SystemController extends BaseController {
	...
	@Autowired
	private TypeUnlessRegionServiceI typeUnlessRegionService;
	...
	@ResponseBody
	public Object typegroupGrid1(TSTypegroup typegroup, HttpServletRequest request, HttpServletResponse response,
			TreeGrid treegrid) {
		...
		typegroup.setTypegroupname(null);
		...
		if (noSearch) {
			cq.isNull("TSTypegroup");
		}

		cq.eq("isDeleted", 0);
		// 排序
		cq.addOrder("orderInLevel", SortDirection.asc);
		cq.add();
		List<TSTypegroup> typegroupList = systemService.getListByCriteriaQuery(cq, false);
		return parseTrees1(typegroupList);
	}

	private List<Map<String, Object>> parseTrees1(List<TSTypegroup> list) {
		List<Map<String, Object>> trees = new ArrayList<Map<String, Object>>();
		for (TSTypegroup entity : list) {
			entity.getOrderInLevel();
			Map<String, Object> tree = new HashMap<String, Object>();
			tree.put("id", entity.getId());
			tree.put("text", entity.getTypegroupname());

			tree.put("code", entity.getTypegroupcode());

			tree.put("typegroupcode", entity.getOrderInLevel());
			tree.put("remarks", entity.getRemarks());
			if (entity.getList() != null && entity.getList().size() > 0) {
				// tree.put("children", parseTrees(entity.getList()));
				List<TSTypegroup> children = entity.getList();
				Collections.sort(children, new Comparator<TSTypegroup>() {
					@Override
					public int compare(TSTypegroup o1, TSTypegroup o2) {
						return o1.getOrderInLevel().compareTo(o2.getOrderInLevel());
					}
				});
				tree.put("children", parseTrees1(children));
			}
			trees.add(tree);
		}
		return trees;
	 ...
	 * @return
	 */
	@RequestMapping(params = "typegroupGrid2")
	@ResponseBody
	public Object typegroupGrid2(TSTypegroup typegroup, HttpServletRequest request, HttpServletResponse response,
			TreeGrid treegrid) {
		CriteriaQuery cq = new CriteriaQuery(TSTypegroup.class);
		...
		List<TSTypegroup> typegroupList = systemService.getListByCriteriaQuery(cq, false);
		TSTypegroup d = new TSTypegroup();
		d.setTypegroupname("空");
		typegroupList.add(0, d);
		return parseTrees11(typegroupList);
	}

	private List<Map<String, Object>> parseTrees11(List<TSTypegroup> list) {
		List<Map<String, Object>> trees = new ArrayList<Map<String, Object>>();
		for (TSTypegroup entity : list) {
			...
			tree.put("text", entity.getTypegroupname());
			if (entity.getList() != null && entity.getList().size() > 0) {
				tree.put("children", parseTrees11(entity.getList()));
			}
			trees.add(tree);
		}
		return trees;
	...
	public ModelAndView goAdd(TSTypegroup typegroup, HttpServletRequest req) {

		int maxNum = 1;

		if (StringUtil.isNotEmpty(typegroup.getId())) {
			typegroup = systemService.getEntity(TSTypegroup.class, typegroup.getId());
			String pid = typegroup.getId();
			req.setAttribute("pid", pid);

			if (pid != null && pid != "") {
				List<TSTypegroup> dt = typegroupService.getChildren(pid);

				if (dt != null && dt.size() > 0) {
					for (int i = 0; i < dt.size(); i++) {

						if (dt.get(i).getOrderInLevel() > 1) {
							maxNum = dt.get(i).getOrderInLevel();
						}
			...
			List<TSTypegroup> tg1 = typegroupService.getAll();

			if (tg1 != null && tg1.size() > 0) {
				for (int i = 0; i < tg1.size(); i++) {

					if (tg1.get(i).getOrderInLevel() > 1) {
						maxNum = tg1.get(i).getOrderInLevel();
					}
		...
		return new ModelAndView("system/type/typegroup-add1");
	}
	...
	@ResponseBody
	public AjaxJson doAdd(TSTypegroup typegroup, HttpServletRequest request) {
		// 设置上级字典
		String pid = request.getParameter("TSTypegroup.id");
		if (pid.equals("")) {
			typegroup.setTSTypegroup(null);
		}
		...
		try {
			// 设置为未删除状态
			typegroup.setIsDeleted(0);

			Long countNum = systemService
					.getCountForJdbc("SELECT count(1) FROM t_s_Typegroup WHERE is_deleted = 0 and typegroupcode = '"
							+ typegroup.getTypegroupcode() + "'");

			if (countNum > 0) {
				message = "字典编码已存在,请重新输入";
				j.setSuccess(false);
			} else {

				// 保存实体到数据库
				systemService.save(typegroup);
	...
	@ResponseBody
	public AjaxJson doDel(TSTypegroup typegroup, HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		typegroup = systemService.getEntity(TSTypegroup.class, typegroup.getId());
		message = "字典表删除成功";
		try {
			List<TSTypegroup> children = typegroupService.getChildren(typegroup.getId());
			
			
			List<TSType> types = typeService.getItems(typegroup.getId());
			
			if ( children.size() > 0 ) {
						// throw new RuntimeException("该该表存在下级，删除失败");
						message = "字典表存在子节点或条目,请先删除子节点或条目";			
				
			}else{
					if(types.size() > 0){
					
						message = "字典表存在字典条目,请先删除条目";
					}else{
					
					// 设置删除状态
					typegroup.setIsDeleted(1);
	...
	public AjaxJson doBatchDel(String ids, HttpServletRequest request) {
		...
		try {
			for (String id : ids.split(",")) {
				TSTypegroup typegroup = systemService.getEntity(TSTypegroup.class, id);
				typegroupService.delete(typegroup);
				systemService.addLog(message, Globals.Log_Type_DEL, Globals.Log_Leavel_INFO);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	@ResponseBody
	public AjaxJson doUpdate(TSTypegroup typegroup, HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		message = "字典表更新成功";

		if ("" == typegroup.getTSTypegroup().getId() || "NULL" == typegroup.getTSTypegroup().getId()) {

			typegroup.setTSTypegroup(null);
		}
		try {
			// 判断 字典编码 是否唯 一
			int countNum = typegroupService.getCountNumUpdate(typegroup.getTypegroupcode(), typegroup.getId());
	...
	public ModelAndView goUpdate(TSTypegroup typegroup, HttpServletRequest req) {
		if (StringUtil.isNotEmpty(typegroup.getId())) {
			typegroup = typegroupService.getEntity(TSTypegroup.class, typegroup.getId());
			req.setAttribute("typegroup", typegroup);
		}
		return new ModelAndView("system/type/typegroup-update1");
	}
	/*****************************type字典条目***************************************/
	
	/**
	 * 方法描述: 查看字典条目列表 作 者： 日 期：
	 * 
	 * @param request
	 * @param departid
	 * @return 返回类型： ModelAndView
	 */
	@RequestMapping(params = "typeList1")
	public ModelAndView typeList1(HttpServletRequest request, String typegroupId) {
		request.setAttribute("typegroupId", typegroupId);

		return new ModelAndView("system/type/typeList1");
	}
	

	@RequestMapping(params = "typeDatagrid1")
	public void typeDatagrid1(TSType type, HttpServletRequest request, HttpServletResponse response,
			DataGrid dataGrid) {
		...
		org.jeecgframework.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, type,
	 ...
	 * @return
	 */
	@RequestMapping(params = "typeTreegrid")
	@ResponseBody
	public Object typeTreegrid(TSType type, HttpServletRequest request, HttpServletResponse response,
			TreeGrid treegrid) {
		...
		cq.addOrder("orderInLevel", SortDirection.asc);
		cq.add();
		List<TSType> typegroupList = systemService.getListByCriteriaQuery(cq, false);
		return typeParseTrees(typegroupList);
	}

	private List<Map<String, Object>> typeParseTrees(List<TSType> list) {
		List<Map<String, Object>> trees = new ArrayList<Map<String, Object>>();
		for (TSType entity : list) {
			entity.getOrderInLevel();
			Map<String, Object> tree = new HashMap<String, Object>();
			tree.put("id", entity.getId());
			tree.put("text", entity.getTypename());

			tree.put("typecode", entity.getTypecode());

			tree.put("isStop", entity.getIsStop());
			tree.put("isSys", entity.getIsSys());
			tree.put("orderInLevel", entity.getOrderInLevel());
			if (entity.getList() != null && entity.getList().size() > 0) {
				// tree.put("children", parseTrees(entity.getList()));
				List<TSType> children = entity.getList();
				Collections.sort(children, new Comparator<TSType>() {
					@Override
					public int compare(TSType o1, TSType o2) {
						return o1.getOrderInLevel().compareTo(o2.getOrderInLevel());
					}
				});
				tree.put("children", typeParseTrees(children));
			}
			trees.add(tree);
		}
		return trees;
	...
	@RequestMapping(params = "typeTreegrid1")
	@ResponseBody
	public Object typeTreegrid1(TSType type, HttpServletRequest request, HttpServletResponse response,
			TreeGrid treegrid) {
		CriteriaQuery cq = new CriteriaQuery(TSType.class);
		String typegroupId = request.getParameter("typegroupId");
		...
		TSType  t = new TSType();
		t.setTypecode("空");
		typegroupList.add(0, t);	
		
		return typeParseTrees1(typegroupList);
	}

	private List<Map<String, Object>> typeParseTrees1(List<TSType> list) {
		List<Map<String, Object>> trees = new ArrayList<Map<String, Object>>();
		for (TSType entity : list) {
			entity.getOrderInLevel();
			Map<String, Object> tree = new HashMap<String, Object>();
			...
			if (entity.getList() != null && entity.getList().size() > 0) {
				...
				tree.put("children", typeParseTrees1(children));
			}
			trees.add(tree);
		}
		return trees;
	 ...
	 * @return
	 */
	@RequestMapping(params = "typeGoAdd")
	public ModelAndView typeGoAdd(TSType type, String typegroupId, HttpServletRequest req) {

		int maxNum = 1;

		if (StringUtil.isNotEmpty(type.getId())) {
			type = systemService.getEntity(TSType.class, type.getId());
			String typePid = type.getId();
			req.setAttribute("pid", typePid);

			if (typePid != null && typePid != "") {
				List<TSType> dt = typeService.getChildren(typePid);
			...
			List<TSType> tg1 = typeService.getAllTypes(typegroupId);
		...
		return new ModelAndView("system/type/type-add1");
	}
	/**
	 * 添加字典条目
	 * 
	 ...
	 * @return
	 */
	@RequestMapping(params = "typeDoAdd")
	@ResponseBody
	public AjaxJson typeDoAdd(TSType type, HttpServletRequest request) {

		// 设置上级字典
				String pid = request.getParameter("TSType.id");
				if (pid.equals("")) {
					type.setTSType(null);
				}
		...
		try {
			...
			// 类型id
			String typegroupId = request.getParameter("TSTypegroup.id");
			
			// 字典条目id
			String itemId = type.getId();
			
			// 判断  区划是否为 空
			// 如果不为空 则 把 区划信息 存到 关联表中。
			// 为空则 不做 记录
			if (orgIds != "" && orgIds != null) {
				String[] orgIds1 = orgIds.split(",");
				for (int i = 0; i < orgIds1.length; i++) {

					TSTypeUnlessRegionEntity typeUnlessRrgion = new TSTypeUnlessRegionEntity();
					typeUnlessRrgion.setDictionaryId(itemId);
					typeUnlessRrgion.setTypeId(typegroupId);
					typeUnlessRrgion.setRegionId(orgIds1[i]);

					typeUnlessRegionService.save(typeUnlessRrgion);
				}
	 ...
	 * @return
	 */
	@RequestMapping(params = "typeGoUpdate")
	public ModelAndView typeGoUpdate(TSType type, HttpServletRequest req) {

		if (StringUtil.isNotEmpty(type.getId())) {
			type = typeService.getEntity(TSType.class, type.getId());
			req.setAttribute("typePage", type);
		
			
			String typegroupId = req.getParameter("typegroupId");
			String typeId = type.getId();
			
			List<TSTypeUnlessRegionEntity> typeUnlessRegionList = typeService.getRegionId(typegroupId, typeId);
			
			if(typeUnlessRegionList != null && typeUnlessRegionList.size() >0){
				
				StringBuffer sb = new StringBuffer();
				StringBuffer sbs = new StringBuffer();
				for (TSTypeUnlessRegionEntity typeUnlessRegion : typeUnlessRegionList) {

					String regionId = typeUnlessRegion.getRegionId();
					if (regionId != null && regionId != "") {
						SystemRegionEntity region = systemService.getEntity(SystemRegionEntity.class,
								typeUnlessRegion.getRegionId());

						sb.append(region.getRegionName() + ",");

						sbs.append(region.getId() + ",");

						req.setAttribute("regionName", sb);
						req.setAttribute("orgIds", sbs);
		...
		return new ModelAndView("system/type/type-update1");
	}
	 ...
	 * @return
	 */
	@RequestMapping(params = "typeDoUpdate")
	@ResponseBody
	public AjaxJson typeDoUpdate(TSType type, HttpServletRequest request) {

		String orgIds = request.getParameter("orgIds");

		String typegroupId = type.getTSTypegroup().getId();

		AjaxJson j = new AjaxJson();

		message = "字典条目修改成功";
		if ("" == type.getTSType().getId() || "NULL" == type.getTSType().getId()) {
		...
		try {
			...
			String dictionaryId = type.getId();
			// 从 权限表中 删除 之前的 区划 记录
			typeService.deletedRegionId(typegroupId, dictionaryId);


				if (orgIds != null && orgIds != "") {
					// 重新 添加 提交的 行政区划 到 权限表中
					String[] orgIds1 = orgIds.split(",");

					for (int i = 0; i < orgIds1.length; i++) {

						TSTypeUnlessRegionEntity typeUnlessRegion = new TSTypeUnlessRegionEntity();
						typeUnlessRegion.setDictionaryId(dictionaryId);
						typeUnlessRegion.setTypeId(typegroupId);
						typeUnlessRegion.setRegionId(orgIds1[i]);

						typeUnlessRegionService.save(typeUnlessRegion);
					}
				}
	 ...
	 * @return
	 */
	@RequestMapping(params = "isAble")
	@ResponseBody
	public AjaxJson isAble(TSType type, HttpServletRequest request) {
		...
		try {
			...
			TSType dt = systemService.getEntity(TSType.class, dictionaryId);

			systemService.addLog(message, Globals.Log_Type_INSERT, Globals.Log_Leavel_INFO);

			Integer number = dt.getIsStop();
			Integer number1 = dt.getIsSys();
			// number 为 0的时候 启用
			if (number == 0 && number1 == 0) {
				dt.setIsStop(1);

				// 1的时候禁用
			} else if (number == 1 && number1 == 0) {

				dt.setIsStop(0);
			}

			// 进行 修改保存到数据库操作
			typeService.saveOrUpdate(dt);
	 ...
	 * @return
	 */
	@RequestMapping(params = "typeDoBatchDel")
	@ResponseBody
	public AjaxJson typeDoBatchDel(String ids, HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		message = "字典条目删除成功";
		try {
			for (String id : ids.split(",")) {
				TSType type = systemService.getEntity(TSType.class, id);
				// 设置删除状态，逻辑删除
				type.setIsDeleted(1);
				typeService.save(type);
	...
	public AjaxJson saveDepart(TSDepart depart, HttpServletRequest request) {
		...
		if (StringUtil.isNotEmpty(depart.getId())) {
			...
			// String orgCode = systemService.generateOrgCode(depart.getId(),
			// pid);
	...
	public String updateAuthority(HttpServletRequest request) {
		...
		List<TSRoleFunctionBase> roleFunctionList = systemService.findByProperty(TSRoleFunctionBase.class, "TSRole.id",
				role.getId());
		...
		if (rolefunction != "") {
			...
			for (String s : roleFunctions) {
				TSRoleFunctionBase rf = new TSRoleFunctionBase();
	...
	public ModelAndView diffDataVersion(HttpServletRequest request, @RequestParam String id1,
		...
		if (datalogEntity1 != null && datalogEntity2 != null) {
			...
			for (String string : set) {
				...
				// update-begin--Author: jg_huangxg Date: 20150723
				// for：修复key找不到的bug
				...
				// update-end--Author: jg_huangxg Date: 20150723
				// for：修复key找不到的bug
