...
import org.jeecgframework.web.cgform.service.impl.config.DbTableDmHandleImpl;
...
//		return name;
	...
	public static DbTableHandleI getTableHandle(Session  session) {
		...
		//添加DM数据库支持
		else if(dialect.contains("org.hibernate.dialect.DmDialect")){
	...
	public static String getDataType(Session session){
		...
		if (dialect.equals("org.hibernate.dialect.MySQLDialect")) {
			...
			dataType="SQLSERVER";
		}else if(dialect.equals("org.hibernate.dialect.DmDialect")){
			dataType = "DM";
			logger.info(dataType+"添加达梦数据库支持 ---------- getDataType方法添加达梦数据库类型判断");
		}
		return dataType;
	}
}
