...
public abstract class GenericBaseCommonDao<T, PK extends Serializable>
	...
	public PageList getPageListBySql(final HqlQuery hqlQuery,
		...
		String sql = hqlQuery.getQueryString();
		...
		//查询总数sql
		String countSql = sql;
		if(countSql.toUpperCase().indexOf("ORDER BY")!=-1){
		...
		int allCounts = count.intValue();*/
		int allCounts = ((Number)countQuery.uniqueResult()).intValue(); 
	...
	public PageList getPageList(final HqlQuery hqlQuery,
		...
		String hql = hqlQuery.getQueryString();
		...
		//查询总数hql
		String countHql = hql;
		if(countHql.toUpperCase().startsWith("FROM ")){
			countHql = "select count(*) " + countHql;
		}else if(countHql.toUpperCase().indexOf(" FROM ")!=-1){
			countHql = "select count(*) " + countHql.substring(countHql.toUpperCase().indexOf(" FROM "));
		}
		if(countHql.toUpperCase().indexOf("ORDER BY")!=-1){
			countHql = countHql.substring(0, countHql.toUpperCase().lastIndexOf("ORDER BY"));
		}
		
		Query countQuery = getSession().createQuery(countHql);
		if (needParameter) {
			countQuery.setParameters(hqlQuery.getParam(),
	...
	public <T> List<T> findByHql(String hql, Map<Integer, Object> param) {
		Query q = getSession().createQuery(hql);
		if (param != null) {
			...
			while(it.hasNext()) {
				Integer i = it.next();
				q.setParameter(i.intValue(), param.get(i));
			}
		}
		return q.list();
	}
