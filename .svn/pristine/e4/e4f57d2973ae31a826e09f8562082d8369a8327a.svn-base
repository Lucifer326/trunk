...
import org.jeecgframework.tag.core.hplus.MultiButtonTag;
...
@Scope("prototype")
@Controller
@RequestMapping("/multibuttonController")
public class MultibuttonController {
	 ...
	 * @return
	 */
	@RequestMapping(params = "multibuttonSourceCode")
	@ResponseBody
	public AjaxJson multibuttonSourceCode(MultiButtonTag multiButtonTag, HttpServletRequest request){
		AjaxJson j = new AjaxJson();
		
		//调用标签类的方法 获取源码Code
		String sourceCode = multiButtonTag.end().toString();
	...
	@ResponseBody
	public AjaxJson save(MultiButtonTag multiButtonTag, HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		...
		//设置属性
		String objectToJson = JSON.toJSONString(multiButtonTag);
