...
<t:formvalid formid="formobj" layout="div" dialog="true" refresh="true" action="functionController.do?saveFunction">
	...
	<div class="form">
        ...
        <input name="functionName" class="inputxt" value="${function.functionName}" datatype="*2-50" errmsg="名称为2-50任意字符">
	...
	<div class="form">
        ...
        <input name = "functionLevel" type="hidden" value = "${function.functionLevel}">
        <select name="showFunctionLevel" id="functionLevel" datatype="*">
            <option value="0" <c:if test="${function.TSFunction == null}">selected="selected"</c:if>>
                <t:mutiLang langKey="main.function"/>
            </option>
            <option value="1" <c:if test="${function.TSFunction.id != null}"> selected="selected"</c:if>>
	...
	<div class="form" id="functionremark"><label class="Validform_label">备注: </label> <input name="functionremark" class="inputxt" value="${function.functionremark}"></div>
	</fieldset>
</t:formvalid> 
</body>
</html>
