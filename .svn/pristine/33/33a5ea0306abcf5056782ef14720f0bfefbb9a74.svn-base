...
public class JdbcDao extends SimpleJdbcTemplate{
	...
	public static final String DM_SQL = "select * from (select row_.*,rownum rownum_ from ({0}) row_ where rownum <= {1}) where rownum_>{2}"; //dm
	...
	public static String jeecgCreatePageSql(String sql, int page, int rows){
		...
		if(jdbcUrl.indexOf(DATABSE_TYPE_MYSQL)!=-1){
			...
			} else if(jdbcUrl.indexOf(DATABSE_TYPE_DM)!=-1) {
				sql = MessageFormat.format(DM_SQL, sqlParam);
