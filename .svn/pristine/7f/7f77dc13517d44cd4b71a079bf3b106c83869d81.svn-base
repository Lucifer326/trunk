...
public class TaskJeecgService{
     ...
     * 2017-09-01
     * 监听测试函数*/
    public void updateStartTest(DelegateTask execution){
    	//将业务数据的流程状态改为已完成
		Object bpm_form_key = execution.getVariable(WorkFlowGlobals.BPM_FORM_KEY);//获取流程对应的表单
		if(oConvertUtils.isNotEmpty(bpm_form_key)){
			//修改业务数据的状态
			...
			Object reason = execution.getVariable("REMARK");//获取流程对应的表单
			activitiDao.updateTest((String)id, (String)bpm_form_key,(String)reason+":Start");
		}
    }
    public void updateEndTest(DelegateTask execution){
    	//将业务数据的流程状态改为已完成
		Object bpm_form_key = execution.getVariable(WorkFlowGlobals.BPM_FORM_KEY);//获取流程对应的表单
		if(oConvertUtils.isNotEmpty(bpm_form_key)){
			//修改业务数据的状态
			...
			Object reason = execution.getVariable("create_by");//获取流程对应的表单
			activitiDao.updateTest((String)id, (String)bpm_form_key,(String)reason+":End");
		}
    }
    //End
}
