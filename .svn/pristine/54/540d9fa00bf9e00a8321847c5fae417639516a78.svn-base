...
<t:formvalid formid="formobj" layout="div" dialog="true" action="functionController.do?saveop">
	<!-- <input name="id" type="hidden" value="${operation.id}">${operation.id} -->
	<fieldset class="step">
		 <div class="form">
            <label class="Validform_label"> 按钮ID: </label>
            <input name="id" class="inputxt"  readonly="readonly" value="${operation.id}">
        </div>
        <div class="form">
        ...
        <select name="status">
            ...
            <label  class="Validform_label">
            	绑定流程 
            	<input name="isBingdingFlow" type="checkbox" onclick="this.value=this.checked?1:0" value="${operation.isBingdingFlow}"
            		<c:if test="${operation.isBingdingFlow eq 1}">checked="checked"</c:if>>
            </label>
            <label  class="Validform_label">
            	材料配置
            	<input name="isMaterialConf" type="checkbox" onclick="this.value=this.checked?1:0" value="${operation.isMaterialConf}"
            		<c:if test="${operation.isMaterialConf eq 1}">checked="checked"</c:if>>
            </label>
            <label  class="Validform_label">
            	绑定规则要素
            	<input name="isBingdingRegulation" type="checkbox" onclick="this.value=this.checked?1:0" value="${operation.isBingdingRegulation}"
            		<c:if test="${operation.isBingdingRegulation eq 1}">checked="checked"</c:if>>
            </label>
        </div>
	</fieldset>
</t:formvalid>
