...
//上级字典条目
  ...
  //选择行政区划
	...
	function openRegionSelect() {
	  ...
	  $.dialog.setting.zIndex = 9999; 
		...
		$.dialog({content: 'url:systemRegionController.do?regionSelect&orgIds='+orgIds, zIndex: 99900, title: '区划列表', 
 ...
 <body>
  <t:formvalid formid="formobj" layout="div" dialog="true" action="systemController.do?typeDoUpdate">
	<input id="id" name="id" type="hidden" value="${typePage.id}">
	<input  name="TSTypegroup.id" type="hidden" class="inputxt" value="${typegroupId}">
	<input name = "isDeleted" type="hidden" value="${typePage.isDeleted}"/>
	<input  name="isSys" type="hidden" class="inputxt" value="${typePage.isSys}">
	<input  name="isStop" type="hidden" class="inputxt" value="${typePage.isStop}">
	<fieldset class="step">
                
      
        <div class="form">
            <label class="Validform_label">条目编码: </label>
            <input  name="typecode" class="inputxt" value="${typePage.typecode}" datatype="*1-50">
            <span class="Validform_checktip">条目编码为1-50个任意字符</span>
        </div>
         <div class="form">
            <label class="Validform_label">条目名称: </label>
            <input name="typename" class="inputxt" value="${typePage.typename}"  datatype="*1-50">
        ...
        <div class="form">
            ...
            <input name="orderInLevel" class="inputxt" value="${typePage.orderInLevel}"  datatype="n1-3">
       ...
       <div class="form">
	      	...
	      	<td align="right"><label class="Validform_label"> 禁用区划: </label></td>
	      	 <div>
                ...
                <input id="regiontname" name="regiontname" style="border-radius:3px;border:1px solid #3DADF5; height:40px; width:150px" type="text"  readonly = "readonly" class="inputxt"  value="${regionName}" onclick="openRegionSelect()">
