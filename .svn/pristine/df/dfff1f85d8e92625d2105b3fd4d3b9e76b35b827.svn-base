...
function InitLeftMenu() {
	...
	// update-start--Author:gaofeng  Date:2014-01-09 for:新增首页风格,一级菜单点击事件的切换操作
	 $(".shortcut li").live("click",function(){
	   ...
	   if ($(this).attr("subFunction") == "none") {
			var subtitle = $(this).attr("subtitle");
			var suburl = $(this).attr("suburl");
			var subicon = $(this).attr("subicon");
			$(this).click(addTab(subtitle, suburl, subicon));
			var navaa = $("#nav .panel");
			navaa.hide();
			$("#layoutA").layout("collapse","west");
		}else{
//			if($("#leftMenu")[0].style.display=='none' ){
			if($("#layoutA").layout("panel","west")[0].style.display=='none' ){
				$("#layoutA").layout("expand","west");
			}
			var navaa = $("#nav .panel");
			navaa.hide();
			navaa.eq($(this).index()).show();// 获取第几个一级菜单，并与之对应
