...
public class Pager {
	...
	public String getToolBar(String url) {
		...
		if (isFirst())
			//str += "第" + curPageNO + "页&nbsp;共" + pageCount + "页&nbsp;首页 上一页&nbsp;";
     		sb.append("第").append(curPageNO).append("页&nbsp;共").append(pageCount).append("页&nbsp;首页 上一页&nbsp;");
		else {
			//str += "第" + curPageNO + "页&nbsp;共" + pageCount + "页&nbsp;<a href='" + url + temp + "curPageNO=1" + conditions + "'>首页</a>&nbsp;";
			sb.append("第").append(curPageNO).append("页&nbsp;共").append(pageCount).append("页&nbsp;<a href='").append(url).append(temp).append("curPageNO=1").append(conditions).append("'>首页</a>&nbsp;");
			//str += "<a href='" + url + temp + "curPageNO=" + previous() + conditions + "' onMouseMove=\"style.cursor='hand'\" alt=\"上一页\">上一页</a>&nbsp;";
			sb.append("<a href='").append(url).append(temp).append("curPageNO=").append(previous()).append(conditions).append("' onMouseMove=\"style.cursor='hand'\" alt=\"上一页\">上一页</a>&nbsp;");
		}
		if (isLast() || rowsCount == 0)
			...
			sb.append("<a href='").append(url).append(temp).append("curPageNO=").append(next()).append(conditions).append("' onMouseMove=\"style.cursor='hand'\" >下一页</a>&nbsp;");
			//str += "<a href='" + url + temp + "curPageNO=" + pageCount + conditions + "'>尾页</a>&nbsp;";
			sb.append("<a href='").append(url).append(temp).append("curPageNO=").append(pageCount).append(conditions).append("'>尾页</a>&nbsp;");
		}
		//str += "&nbsp;共" + rowsCount + "条记录&nbsp;";
		sb.append("&nbsp;共").append(rowsCount).append("条记录&nbsp;");
		//str += "&nbsp;转到<select name='page' onChange=\"location='" + url + temp + "curPageNO='+this.options[this.selectedIndex].value\">"; 
		sb.append("&nbsp;转到<select name='page' onChange=\"location='").append(url).append(temp).append("curPageNO='+this.options[this.selectedIndex].value\">");
	...
	public String getToolBar(String myaction, String myform) {
		...
		sb.append("function commonSubmit(val){").append("\n");
		// 校验是否全由数字组成
		//str += "var patrn=/^[0-9]{1,20}$/;" + "\n";
		sb.append("var patrn=/^[0-9]{1,20}$/;").append("\n");
		//str += "if (!patrn.exec(val)){" + "\n";
		sb.append("if (!patrn.exec(val)){").append("\n");
		//str += " alert(\"请输入有效页号！\");" + "\n";
		sb.append(" alert(\"请输入有效页号！\");").append("\n");
		//str += " return false ;" + "\n";
		sb.append(" return false ;").append("\n");
		//str += " }else{" + "\n";
		sb.append(" }else{").append("\n");
		//str += "    document." + myform + ".action='" + myaction + "&curPageNO='+val;" + "\n";
		sb.append("    document.").append(myform).append(".action='").append(myaction).append("&curPageNO='+val;").append("\n");
		//str += "    document." + myform + ".submit();" + "\n";
		sb.append("    document.").append(myform).append(".submit();").append("\n");
		//str += "    return true ;" + "\n";
		...
		sb.append("&nbsp;<DIV class=pageArea id=pageArea>共<b>").append(rowsCount).append("</b>条&nbsp;当前第").append(curPageNO).append("/").append(pageCount).append("页&nbsp;&nbsp;&nbsp;");
		if (curPageNO == 1 || curPageNO == 0)
		    ...
		    sb.append("<a class=pageFirstDisable title=首页 onMouseMove=\"style.cursor='hand'\">&nbsp;<a class=pagePreviousDisable title=上一页 onMouseMove=\"style.cursor='hand'\"></a>");
		else {
			//str += "<a class=pageFirst title=首页 onMouseMove=\"style.cursor='hand'\" onclick=\"commonSubmit(1)\"></a>";
			sb.append("<a class=pageFirst title=首页 onMouseMove=\"style.cursor='hand'\" onclick=\"commonSubmit(1)\"></a>");
			//str += "<a class=pagePrevious title=上一页 onMouseMove=\"style.cursor='hand'\" onclick=\"commonSubmit(" + (curPageNO - 1) + ")\"></a>";
			sb.append("<a class=pagePrevious title=上一页 onMouseMove=\"style.cursor='hand'\" onclick=\"commonSubmit(").append((curPageNO - 1)).append(")\"></a>");
		}
		if (curPageNO - pageCount == 0 || pageCount == 0 || pageCount == 1)
			...
			sb.append("<a class=pageNext title=下一页 onMouseMove=\"style.cursor='hand'\" onclick=\"commonSubmit(").append((curPageNO + 1)).append(")\"></a>");
			//str += "<a class=pageLast title=尾页 onMouseMove=\"style.cursor='hand'\" onclick=\"commonSubmit(" + pageCount + ")\"></a>";
			sb.append("<a class=pageLast title=尾页 onMouseMove=\"style.cursor='hand'\" onclick=\"commonSubmit(").append(pageCount).append(")\"></a>");
		}
		...
		if (pageCount == 1 || pageCount == 0) {
			...
			sb.append(" &nbsp;转到:<input class=SmallInput type=text style=TEXT-ALIGN: center maxLength=5 name=\"pageroffsetll\" size=3 onKeyPress=\"if (event.keyCode == 13) return commonSubmit(document.all.pageroffsetll.value)\" > 页&nbsp;");
			//str += "<A class=pageGoto id=pageGoto title=转到 onclick='return commonSubmit()'></A></DIV>";
			sb.append("<A class=pageGoto id=pageGoto title=转到 onclick='return commonSubmit()'></A></DIV>");
		} else {
			//str += " &nbsp;转到:<input class=SmallInput type=text style=TEXT-ALIGN: center maxLength=5 name=\"pageroffsetll\" size=3 onKeyPress=\"if (event.keyCode == 13) return commonSubmit(document.all.pageroffsetll.value)\" > 页&nbsp;";
			sb.append(" &nbsp;转到:<input class=SmallInput type=text style=TEXT-ALIGN: center maxLength=5 name=\"pageroffsetll\" size=3 onKeyPress=\"if (event.keyCode == 13) return commonSubmit(document.all.pageroffsetll.value)\" > 页&nbsp;");
			//str += "<A class=pageGoto id=pageGoto title=转到 onclick='commonSubmit(document.all.pageroffsetll.value)'></A></DIV>";
			sb.append("<A class=pageGoto id=pageGoto title=转到 onclick='commonSubmit(document.all.pageroffsetll.value)'></A></DIV>");
		}
		return sb.toString();
	}
}
