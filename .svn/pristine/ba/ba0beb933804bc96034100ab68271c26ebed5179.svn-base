		...
		onSelect : function(file) {
			...
			for ( var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					//modify by zdl 20170906 Reason:由于confirm与平台css样式冲突，导致此确认框失效，因此做一处理 Start
					/*
					...
					//modify by zdl 20170906 Reason:由于confirm与平台css样式冲突，导致此确认框失效，因此做一处理 End
					
				}
			}

			//modify by zdl 20170906 Reason:添加服务端文件重名校验 Start
				...
				if(fId!=null && fId!=""){
					$.ajax({
						...
						success : function(res) {
							if (res == 1) {//服务器存在同名文件
								alert("已存在同名文件：" + file.name);
								thisObj.cancelUpload(file.id);
								thisObj.queueData.filesCancelled++;
								isCancel = true;
								return false;
							}
						}
			...
			//modify by zdl 20170906 Reason:添加服务端文件重名校验 End
			...
			// Run the default event handler
			if ($.inArray('onSelect', settings.overrideEvents) < 0 && !isCancel) {
