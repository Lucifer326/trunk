...
public class MutiLangController extends BaseController {
	...
	public void datagrid(MutiLangEntity mutiLang, HttpServletRequest request,
		...
		String langContext = request.getParameter("langContext");
		if (StringUtil.isNotEmpty(langKey)){
			cq.like("langKey", "%" + langKey + "%");
		}
		if (StringUtil.isNotEmpty(langContext)){
			cq.like("langContext", "%" + langContext + "%");
		}
		cq.add();
		// 查询条件组装器
