...
import org.jeecgframework.core.util.MySessionContext;
...
public class AuthInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
		...
		if (client == null) {
			/*client = ClientManager.getInstance()
					.getClient(request.getParameter("sessionId"));*/
			
			/*update-start------>修复邮箱功能附件上传时firefox浏览器上传失败*/
			HttpSession mySession = MySessionContext.getInstance().getSession(request.getParameter("sessionId"));
			if(mySession==null){
				client=null;
			}else{
				client = (Client)mySession.getAttribute(mySession.getId());
			}
			/*update-end------>修复邮箱功能附件上传时firefox浏览器上传失败*/
			
		}
		
		TSUser user = ResourceUtil.getSessionUserName();
		if(user!=null && Globals.User_ADMIN == user.getStatus()) {
			return true;
		}
		...
		if (excludeUrls.contains(requestPath)) {
			...
			if (client != null && client.getUser() != null) {
				...
				String functionId = "";
				// update-begin--Author:JueYue Date:20140831 for：onlinecodeing
				// 的URL判断--------------------
				...
				// update-begin--Author:许国杰 Date:20151219 for：#813
				// 【online表单】扩展出三个请求：独立的添加、查看、编辑请求，原来的保留
				...
				// update-end--Author:许国杰 Date:20151219 for：#813
				// 【online表单】扩展出三个请求：独立的添加、查看、编辑请求，原来的保留

				// update-end--Author:JueYue Date:20140831 for：onlinecodeing
				// 的URL判断--------------------
				// 这个地方用全匹配？应该是模糊查询吧
				// TODO
				...
				functionId = request.getParameter("clickFunctionId");
				
				
				//获取当前位置信息
				String currentLocation = systemService.getFunctionFullPathById(functionId);
				...
				if (!oConvertUtils.isEmpty(functionId)) {
					...
					Set<String> operationCodes = systemService
							.getOperationCodesByUserIdAndFunctionId(
									client.getUser().getId(),dept.getId(), functionId);
					// update-begin--Author:zhoujf Date:20150804 for：流程节点页面控件权限
					if (!oConvertUtils.isEmpty(processnodeId)) {
						Set<String> nodeOperationCodes = activitiService
								.getNodeOperaCodesByNodeIdAndFunctionId(processnodeId,
										functionId);
				...
				if (!oConvertUtils.isEmpty(functionId)) {
					...
					// List<String>
					// allOperation=this.systemService.findListbySql("SELECT
					// operationcode FROM t_s_operation WHERE
					// functionid='"+functionId+"'");
					List<TSOperation> allOperation = this.systemService.findByProperty(
							TSOperation.class, "TSFunction.id", functionId);
					...
					if (allOperation.size() > 0) {
						...
						TSDepart dept = (TSDepart) session.getAttribute(ResourceUtil.LOCAL_CLINET_USER_DEPART);
						String hasOperSql = "SELECT fun.id FROM t_s_role_function fun, t_s_role_user role WHERE  "
								+ "fun.functionid='" + functionId
								+ "' AND fun.operation is not null  AND fun.roleid=role.roleid AND role.userid='"
								+ client.getUser().getId() + "' and role.orgid = '" + dept.getId() +"'";
						// ---author:jg_xugj----end-----date:20151210--------for：#781
						// 【oracle兼容】兼容问题fun.operation!='' 在oracle 数据下不正确
						List<String> hasOperList = this.systemService
								.findListbySql(hasOperSql);
						for (String roleFunctionID : hasOperList) {
							List<TSRoleFunction> roleFunctions = this.systemService
									.findByProperty(TSRoleFunction.class, "id",
											roleFunctionID);
							String operationIds = roleFunctions.get(0).getOperation();
						...
						// update-begin--Author:zhoujf Date:20150804
						// for：流程节点页面按钮权限
						if (!oConvertUtils.isEmpty(processnodeId)) {
							Set<String> nodeOperationCodes = activitiService
									.getNodeOperaCodesByNodeIdAndFunctionId(processnodeId,
						...
						// update-end--Author:zhoujf Date:20150804
						// for：流程节点页面按钮权限
					}
					...
					Set<String> dataruleCodes = systemService
							.getOperationCodesByUserIdAndDataId(client.getUser().getId(),dept.getId(),
									functionId);
					// update-begin--Author:zhoujf Date:20150804
					// for：流程节点数据规则权限获取
					if (!oConvertUtils.isEmpty(processnodeId)) {
						Set<String> nodeDataruleCodes = activitiService
								.getOperationCodesByNodeIdAndruleDataId(processnodeId,
										functionId);
					...
					for (String dataRuleId : dataruleCodes) {
						TSDataRule dataRule = systemService.getEntity(TSDataRule.class,
								dataRuleId);
						MENU_DATA_AUTHOR_RULES.add(dataRule);
						MENU_DATA_AUTHOR_RULE_SQL += SysContextSqlConvert
								.setSqlModel(dataRule);

					}
					JeecgDataAutorUtils.installDataSearchConditon(request,
							MENU_DATA_AUTHOR_RULES);// 菜单数据规则集合
					JeecgDataAutorUtils.installDataSearchConditon(request,
							MENU_DATA_AUTHOR_RULE_SQL);// 菜单数据规则sql
	...
	private boolean hasMenuAuth(HttpServletRequest request) {
		...
		if (functionList == null) {
			...
			functionList = systemService
					.findHql("from TSFunction where functionType = ? ", (short) 0);
		...
		TSUser currLoginUser = ClientManager.getInstance()
				.getClient(ContextHolderUtils.getSession().getId()).getUser();
		TSDepart depart = (TSDepart) ContextHolderUtils.getSession().getAttribute(ResourceUtil.LOCAL_CLINET_USER_DEPART);
		boolean b = functionService.menuAuthValidateByUrl(currLoginUser.getId(), depart.getId(), requestPath);
		return b;
	}
