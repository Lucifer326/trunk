...
<div class="easyui-layout" fit="true">
	<div region="center" style="padding:0px;border:0px">
		<t:datagrid name="runningList" title="process.instance.manage" autoLoadData="true" actionUrl="processInstanceController.do?runningProcessDataGrid" sortName="userName" fitColumns="true" treegrid="true" idField="id" fit="true" queryMode="group">
			...
			<t:dgFunOpt exp="isSuspended#ne#&&isSuspended#ne#true&&isSuspended#ne#finished" funname="suspendProcessInstance(processInstanceId,isSuspended)" title="process.pause"></t:dgFunOpt>
			<t:dgFunOpt exp="isSuspended#ne#&&isSuspended#ne#false&&isSuspended#ne#finished" funname="startProcessInstance(processInstanceId,isSuspended)" title="common.start"></t:dgFunOpt>
			<t:dgFunOpt exp="isSuspended#ne#&&isSuspended#ne#finished" funname="closeProcessInstance(processInstanceId,isSuspended)" title="common.close"></t:dgFunOpt>
			<t:dgFunOpt exp="isSuspended#ne#&&isSuspended#ne#true&&isSuspended#ne#finished" funname="reassign(taskId,isSuspended)" title="process.appoint"></t:dgFunOpt>
			<t:dgFunOpt exp="isSuspended#ne#" funname="goProcessHisTab(processInstanceId,activityName)" title="common.history"></t:dgFunOpt>
			<t:dgFunOpt exp="isSuspended#ne#" funname="skipNode(taskId,isSuspended)" title="process.jump"></t:dgFunOpt>
		</t:datagrid>
	</div>
</div>
...
$(document).ready(function(){
	//给时间控件加上样式
		$("#runningListtb").find("input[name='starttime_begin']").attr("style","width:125px;background:#fff url(plug-in/My97DatePicker/skin/datePicker.gif) no-repeat right;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});});
		$("#runningListtb").find("input[name='starttime_end']").attr("style","width:125px;background:#fff url(plug-in/My97DatePicker/skin/datePicker.gif) no-repeat right;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});});
		$("#runningListtb").find("input[name='endtime_begin']").attr("style","width:125px;background:#fff url(plug-in/My97DatePicker/skin/datePicker.gif) no-repeat right;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});});
		$("#runningListtb").find("input[name='endtime_end']").attr("style","width:125px;background:#fff url(plug-in/My97DatePicker/skin/datePicker.gif) no-repeat right;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});});
...
function callbackUserSelect() {
	...
	function reassign(taskId, isSuspended) {
		createwindow('<t:mutiLang langKey='process.appoint'></t:mutiLang>', 'processInstanceController.do?reassignInit&taskId=' + taskId, 700, 100);
	}
