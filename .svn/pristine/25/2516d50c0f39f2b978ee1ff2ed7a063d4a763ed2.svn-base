...
function getzIndex(flag){
	...
	if(zindexNumber == null || isNaN(zindexNumber)){
 ...
 * @returns
 */
function handle(title,url, id,width,height,isRestful) {
	gridname=id;
	var rowsData = $('#'+id).datagrid('getSelections');
	if (rowsData.length > 0) {
		for (var i = 0; i < rowsData.length; i++) {
			var wrIspublish = rowsData[i].handleStatus;
			if (wrIspublish == '1' ) {
				tip('该信访记录已经办理，无需再办理！');
				return;
			}
...
function addOneTab(subtitle, url, icon) {
	//window.top.addTab(subtitle,url);
	var contextPath = document.location.pathname;   
	var index =contextPath.substr(1).indexOf("/"); 
	//这个地方可能有问题，要根据具体项目适当修改  
	contextPath = contextPath.substr(0,index+1); 
	var indexStyle = getCookie("JEECGINDEXSTYLE_"+contextPath);
    ...
    //}
	 if (indexStyle.indexOf('salvation')!=-1){
			window.top.addTab(subtitle,url);
			return;
	...
	}else if(indexStyle.indexOf("hplus")!=-1){
		/*var id = "";
		id = createTabId(subtitle);
		window.top.addTabs({id:id,title:subtitle,close: true,url: url});*/
		var str = '<a href="javascript:;" class="active J_menuTab" data-id="' + url + '">流程配置 <i class="fa fa-times-circle"></i></a>';
		parent.$('.J_menuTab').removeClass('active');
		
        // 添加选项卡对应的iframe
        var str1 = '<iframe class="J_iframe" name="iframe' + icon + '" width="100%" height="100%" src="' + url + '" frameborder="0" data-id="processController.do?processTabs&processid=' + icon + '"seamless="seamless"></iframe>';
...
function doProcess(data){       
	  if(data.success==true){
		  parent.doProcess();//调用流程任务的方法
	  }
	 
}
//检查当前登录人是否有审批流程
function isHashFlow(fId,buttonId){
	var b=true;
	$.ajax({
		url : "bsprFamTempController.do?isHashFlow",
		async:false,
		data:{"functionId":fId,"buttonId":buttonId},
		type:'post',
		dataType:"json",
		success : function(data) {
			if(data.success){
