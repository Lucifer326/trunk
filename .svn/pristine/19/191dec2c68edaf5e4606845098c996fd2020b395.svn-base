...
public class ConfigServiceImpl implements ConfigServiceI {
	 ...
	 */
	@Ehcache
	public Map<String, Object> queryConfigs_Horse(String tableName,String jversion) {
		//step.1 要返回的配置数据
	 ...
	 * 2017-08-30
	 */
	private void loadConfigs(Map<String, Object> configs, CgFormHeadEntity tableEntity,String processKey) {
		//获取动态表明细配置
		...
		//2017-08-30
		//Start
		configs.put(CgAutoListConstant.PROCESS_KEY,processKey);
		//End
	 ...
	 */
	@Ehcache
	public Map<String, Object> queryConfigs(String tableName,String jversion,String processKey) {
		//step.1 要返回的配置数据
		Map<String, Object> configs = new HashMap<String,Object>();
		//step.2 获取动态表配置
		CgFormHeadEntity tableEntity = null;
		try{
			tableEntity = tablePropertyService.findByProperty(CgFormHeadEntity.class, "tableName", tableName).get(0);
			loadConfigs(configs,tableEntity,processKey);
		}catch (Exception e) {
			e.printStackTrace();
			throw new RuntimeException("没有找到该动态列表");
		}
