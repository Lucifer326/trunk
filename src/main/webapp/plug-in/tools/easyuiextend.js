...
/**
 * 打开流程办理页面 - New (新的流程办理页面包括"表单信息","任务处理","流程跟踪") 签收 待办二和一
 */
function claimAndgoTaskTab(taskId, title, width, height) {
	var b = false;
	$.ajax({
		async : false,
		cache : false,
		type : 'POST',
		data : {"taskId":taskId},
		url : 'activitiController.do?isHasClaimAndgoTask',// 请求的action路径
	...
	if(b)
	{
		...
		var url = 'taskController.do?claimAndgoTaskTab&taskId=' + taskId;
		$.dialog({
			content: 'url:'+url,
			title : '流程办理--当前环节 : ' + title,
			lock : true,
...
function selectEntruster(taskId, title) {
	...
	createwindow('选择转办人' + title, 'activitiController.do?goEntrustAdd&tag=task&taskId=' + taskId,700,150);
}
