		...
		//解决大字段问题 
			...
			if($("#id").val() != ""){
				$("#tab_div_database tr").each(function(){
					var stext=$(this).find('select').find("option:selected").text();
					if(stext == "Text" || stext == "Date"|| stext == "Blob" || stext == "Clob"){
						$(this).find('select').html("<option value='" + stext + "'>" + stext + "</option>");
					}
...
function fixTab(){
	...
	$('.t_table').height($(window).height()-405);
