(function($, win, undef) {
	...
	Validform.util = {
		dataType : {
			...
			"url" : /^(\w+:\/\/)?\w+(\.\w+)+.*$/,
			"w1" : /^([a-zA-Z]+)([\w]+)*$/,
			"zj":function(gets,obj,curform,regxp){
				var checkvalue=gets;//需要验证的值
				var zjparam=obj.attr("dataType");//zj这个规则里的用户定义的规则
				/**取依赖的字段名和用户定义的对应规则**/
				var mid = zjparam.substr(zjparam.indexOf("(",1)+1);
				var colpara = mid.substr(0,mid.length-1);
				/**取依赖的字段名和用户定义的对应规则**/
				
				/**zj 规则的组成的 第一个为字段名，之后为 值#@#表达式  依赖的字段名*@*依赖的字段值1#@#正则表达式1*@*依赖的字段值2#@#正则表达式2*@*依赖的字段值3#@#正则表达式3    
				 * 例子：colpara=                          zjtype*@*身份证号#@#/^[\w\W]{6,16}$/*@*军官证#@#/^(\w+:\/\/)?\w+(\.\w+)+.*$/*@*社保号#@#/^(\w+:\/\/)?\w+(\.\w+)+.*$/    
				 **/
				var strs = colpara.split("*@*"); 
				var ylcolumn=strs[0];//依赖的字段名
				var selectvalueDom=$("select[name='"+ylcolumn+"']");
				var selectvalue = $("select[name='"+ylcolumn+"']").val(); //选中的下拉列表value值
				var selecttext = $("select[name='"+ylcolumn+"'] option:selected").text().replace(/(^\s*)|(\s*$)/g, "");//选中的下拉列表text文本
				var zjpass=false; 
				if(!$.trim(selectvalue)==""){
					for(var p=1;p<strs.length;p++){
						var zz=strs[p].split("#@#");
						var zjmc = zz[0].replace(/(^\s*)|(\s*$)/g, "");//依赖的字段值
						var zzbds = zz[1];//表达式
						if(zjmc==selecttext){
							var re= eval(zzbds);
							zjpass=re.test(checkvalue);
							tiptitle=selecttext;
							break;
						}
					}
					...
					if(gets!=""){
						tiptitle="请选择"+selectvalueDom.siblings(".Validform_label").text();
						return false;
					}else{
						tiptitle="";
						return true;
		...
		_regcheck : function(datatype, gets, obj, curform) {
			...
			return {
				passed : passed,
				type : type,
				info : passed?info:datatype=="zj"?gets==""?"请填写"+tiptitle+"!":tiptitle.indexOf("请选择")!=-1?tiptitle:info+tiptitle:info
			};
		},
		regcheck : function(datatype, gets, obj) {
			midDatatype = datatype;
			midDatatype = midDatatype.substr(0,midDatatype.indexOf("(",1));
			if(midDatatype=="zj"){
				datatype = midDatatype;
			} 
