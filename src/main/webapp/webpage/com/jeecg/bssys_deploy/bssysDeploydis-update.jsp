...
<body>
	<t:formvalid formid="formobj" dialog="true" usePlugin="password"
		layout="table" action="bssysDeploydisController.do?doUpdate" tiptype="1">
		...
		<table class="table-easyui" cellpadding="0" cellspacing="0">
			<tbody>
				...
				<tr>
					...
					<td><c:choose>
							<c:when test="${bssysDeploydisPage.controlType==2}">
								<input id="value_int" name="value" type="text"
									style="width: 400px" class="inputxt" datatype="n1-50"
									value='${bssysDeploydisPage.value}'>
							</c:when>
							<c:when test="${bssysDeploydisPage.controlType==3}">
								<select id="value_select" name="value" style="width: 400px"
									value="${bssysDeploydisPage.value}"></select>
							</c:when>
							<c:when test="${bssysDeploydisPage.controlType==4}">
								<div id="div_date">
									<input id="value_date" name="value" type="text"
										style="width: 400px" class="easyui-datebox"
										value='${bssysDeploydisPage.value}' nullmsg="请填写配置日期！">
								</div>
							</c:when>
							<c:otherwise>
								<!-- 1：字符型 -->
								<textarea id="value_txt" name="value" style="width: 400px"
									class="inputxt" datatype="s1-50" rows="3">${bssysDeploydisPage.value}</textarea>
							</c:otherwise>
						</c:choose></td>
				</tr>
			</tbody>
