...
function build(obj) {
	...
	newTdObj6.innerHTML = "<input readonly type='text' value='" + obj.price + "' name='price' class='form-control fl' />";
...
function verification(page, formId) {
	...
	if ($(form).find("input[name=sub_name]").val() == undefined) {
		layer.msg("请新建物资信息", {time: 2000, icon: 7, shift: 1}, function () {
		});
		return false;
	}
	...
	$.each($(form).find("input[name=sub_name]"), function () {
		...
		if ($.trim($(this).parent().parent().find("input[name=messageid]").val()) == "undefined") {
			layer.msg("物资信息错误,请重新选择", {time: 2000, icon: 7, shift: 1}, function () {
			});
			page.$($(this).parent().parent().find("input[name=sub_name]")).focus();
			flag = false;
			return false;
		}
		if ($(this).parent().parent().find("input[name=price]").val() <= 0 || $.trim($(this).parent().parent().find("input[name=price]").val()) == "undefined") {
			layer.msg("物资信息错误,请重新选择", {time: 2000, icon: 7, shift: 1}, function () {
			});
			page.$($(this).parent().parent().find("input[name=price]")).focus();
			flag = false;
			return false;
		}
		...
		if ($.trim($(this).parent().parent().find("input[name=manufactureDate]").val()) > $(this).parent().parent().find("input[name=purchaseDate]").val()) {
