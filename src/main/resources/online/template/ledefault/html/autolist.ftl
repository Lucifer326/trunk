		...
		[	
					...
					<#list config_fieldList  as x>  
						 {	field:'${x['field_id']}',
					 		...
					 		formatter:function(value,rec,index){
					 			var href = '';
					 			if(value){
					 				var dt = new Date(value.replace(/-/,"/"));
					 				href += dt.getFullYear()+'-'+dt.getMonth()+'-'+dt.getDate();
					 			}
						 		return href;
					 		},
						 	</#if>
			...
			{field:'opt',title:'操作',width:200,formatter:function(value,rec,index){
						...
						if(rec.bpm_status!=null&&rec.bpm_status.indexOf('提交')>0){
							href+="[<a href='#' onclick=confirm('activitiController.do?startOnlineProcess&processkey=${config_processkey}&configId=${config_id}&id="+rec.id+"','确认提交流程','${config_id}List')>";
							href+="提交流程</a>]";
						}
