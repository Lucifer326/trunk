...
public class ${entityName}ServiceImpl extends CommonServiceImpl implements ${entityName}ServiceI {
	...
	public void updateMain(${entityName}Entity ${entityName?uncap_first},
		...
		for(${sub.entityName}Entity oldE:${sub.entityName?uncap_first}OldList){
			...
			String hql1 = "delete from ${sub.entityName}Entity where 1 = 1<#list sub.foreignKeys as key> AND ${key?uncap_first} = ? </#list>";
			commonDao.executeHql(hql1,<#list sub.foreignKeys as key><#if key?lower_case?index_of("${jeecg_table_id}")!=-1>${jeecg_table_id}${sub_index}<#else>${key?uncap_first}${sub_index}</#if><#if key_has_next>,</#if></#list>);
		}
		</#list>
		//执行更新操作配置的sql增强
