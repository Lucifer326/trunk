...
<head>
 ...
 <#-- update--end--author:zhangjiaqiang date:20170522 for:ueditor配置文件只加载一次 -->
  <form id="formobj"  class="form-horizontal m-t" role="form" action="${entityName?uncap_first}Controller.do?doUpdate" name="formobj" method="post">
		<input type="hidden" id="btn_sub" class="btn_sub"/>
		<#list columns as po>
			<#if po.isShow == 'N'>
				<#if po.type='java.util.Date'>
					<input id="${po.fieldName}" name="${po.fieldName}" type="hidden" value="<fmt:formatDate value='${'$'}{${entityName?uncap_first}Page.${po.fieldName} }' pattern='yyyy-MM-dd HH:mm:ss'/>">
				<#else>
				...
				<#list pageColumns as po>
					...
					<label class="col-sm-2 control-label">${po.content}：</label>
						...
						<#elseif po.showType=='select' || po.showType=='checkbox' || po.showType=='list'>	 
							<div class="col-sm-4">
								<h:dictSelect field="${po.fieldName}" type="${po.showType?if_exists?html}" extendJson="{class:'form-control'}"
								<#if po.dictTable?if_exists?html != ''>dictTable="${po.dictTable?if_exists?html}" dictField="${po.dictField?if_exists?html}" dictText="${po.dictText?if_exists?html}"<#else>typeGroupCode="${po.dictField}"</#if> defaultVal="${'$'}{${entityName?uncap_first}Page.${po.fieldName}}" hasLabel="false"  title="${po.content}"></h:dictSelect>
								<span class="Validform_checktip" style="float:left;height:0px;"></span>
								<label class="Validform_label" style="display: none">${po.content?if_exists?html}</label>      
							</div>
						<#elseif po.showType=='date'>
							<div class="col-sm-4">
								<input id="${po.fieldName}" name="${po.fieldName}" type="text"   class="form-control" onClick="laydate()" <#if po.fieldValidType?if_exists?html != ''> datatype="${po.fieldValidType?if_exists?html}"<#else><#if po.isNull != 'Y'>datatype="*"</#if></#if>  value='<fmt:formatDate value='${'$'}{${entityName?uncap_first}Page.${po.fieldName}}'  type="date" pattern="yyyy-MM-dd"/>'>
								<span class="Validform_checktip" style="float:left;height:0px;"></span>
								<label class="Validform_label" style="display: none">${po.content?if_exists?html}</label>
					    	</div>
...
function ajaxLoadFiles(){
	...
	$.ajax({
	   type: "post",
	   url: "cgUploadController.do?getFiles&cgFormId=" +  cgFormId,
	   success: function(data){
		...
		$.each(arrayFileObj,function(n,file){
			...
			var td_title = $("<td>" + file.title + "</td>")
	  		...
	  		var td_del = $("<td><a href=\"javascript:void(0)\" class=\"jeecgDetail\" onclick=\"removeFile(this,'" + file.fileKey + "')\">删除</a></td>");
	  		tr.appendTo(table);
	  		td_title.appendTo(tr);
	  		td_download.appendTo(tr);
	  		td_del.appendTo(tr);
	  		addFileToArray(file.title);
		 });
	   }
	});
}
...
function removeFile(obj,fileId){
	$(obj).parent().parent().remove();
	var delFile = document.getElementById("delFileIds");
	delFile.value = delFile.value + "," + fileId;
}
    ...
    $("#formobj").Validform({
        ...
        callback: function(data) {
            if (data.success == true) {
				...
				var delIds = "";
				if(document.getElementById("delFileIds")){
					delIds = document.getElementById("delFileIds").value;
				}
				if(hasFiles > 0 || delIds!=""){
					myUpload();
				}else {
					var win = parent;
		            win.reloadTable();
