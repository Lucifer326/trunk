				...
				for (Iterator j = employee.elementIterator(); j.hasNext();) {
					...
					for (int k = 0; k < fields.length; k++) {
						if (node.getName().equals(fields[k].getName())) {
							...
							sb.append("set").append(stringLetter).append(fieldName.substring(1));
							Method setMethod = entityClass.getMethod(sb.toString(), new Class[] { fields[k].getType() });
...
//            update-end--Author:zhangguoming  Date:20140819 for：递归子节点属性
	...
	public List<TreeGrid> treegrid(List all, TreeGridModel treeGridModel) {
		...
		for (Object obj : all) {
			...
			if(treeGridModel.getRemark()!=null){
