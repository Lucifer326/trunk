...
import org.jeecgframework.web.system.pojo.base.QueryCondition;
			...
			try {
				...
				if (type.contains("class java.lang")
					...
					if (value != null && !value.equals("")) {
						...
						}else if(HqlRuleEnum.IN.equals(rule)){//增加else,否则这是单独的if,如果rule是LIKE,会进入这个if的else，造成最终的rule被改变赋值为EQ;
							//Horse_Little
							//2017-09-06
							//增加对In关系符的处理
							if ("class java.lang.Integer".contains(type)){
								value="("+String.valueOf(value.toString())+")";
							} else{
								//value="('"+String.valueOf(value.toString()).replace(",","','")+"')";
								value=String.valueOf(value.toString());
							}
						} else {
							rule = HqlRuleEnum.EQ;
