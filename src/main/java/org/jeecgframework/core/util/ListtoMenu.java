...
import org.jeecgframework.core.enums.SysFAIconEnum;
...
public class ListtoMenu {
		...
		for (TSFunction node : set1) {
...

			{
				buffer.append("{\"menuid\":\"" + node.getId() + " \",\"icon\":\""
						+ TSIcon.allTSIcons.get(node.getTSIcon().getId()).getIconClas() + "\"," + "\"menuname\":\""
	...
	public static String getEasyuiMenu(List<TSFunction> pFunctions, List<TSFunction> functions) {
		...
		for (TSFunction pFunction : pFunctions) {
			menuString.append("<div  title=\"" + getMutiLang(pFunction.getFunctionName()) + "\" iconCls=\""
					+ TSIcon.allTSIcons.get(pFunction.getTSIcon().getId()).getIconClas() + "\">");
			/*
			 * int submenusize = pFunction.getSubFunctionSize(); if (submenusize
	...
	// update-start--Author:zhangguoming Date:20140622
	// for：左侧树调整：加大宽度、更换节点图标、修改选中颜色
	...
	public static String getEasyuiMultistageTree(Map<Integer, List<TSFunction>> map, String style) {
		...
		if ("easyui".equals(style)) {
			...
			for (TSFunction function : list) {
				 ...
				 * "<div title=\"" + getMutiLang(function.getFunctionName()) +
				 * "\" iconCls=\"" +
				 * TSIcon.allTSIcons.get(function.getTSIcon().getId()).
				 * getIconClas() +
				 * "\"><ul class=\"easyui-tree tree-lines\"  fit=\"false\" border=\"false\">"
				 * +getLeafOfTree(function) +"</ul></div>"); }else{
				 * menuString.append("<div   title=\"" +
				 * getMutiLang(function.getFunctionName()) + "\" iconCls=\"" +
				 * TSIcon.allTSIcons.get(function.getTSIcon().getId()).
				 * getIconClas() + "\">"); menuString.append(
				 * "<ul class=\"easyui-tree tree-lines\"  fit=\"false\" border=\"false\">"
				 * ); menuString.append(getChildOfTree(function,1,map));
				 * menuString.append("</ul></div>"); }
				 */
	...
	// update-end--Author:zhangguoming Date:20140622
	// for：左侧树调整：加大宽度、更换节点图标、修改选中颜色
		...
		for (TSFunction function : list) {
			if (function.getTSFunction().getId().equals(parent.getId())) {
				if (!function.hasSubFunction(map)) {
					...
					menuString.append("<div title=\"" + getMutiLang(function.getFunctionName()) + "\" iconCls=\""
							+ TSIcon.allTSIcons.get(function.getTSIcon().getId()).getIconClas() + "\"><ul>");
			...
			if (function.getTSFunction().getId().equals(parent.getId())) {
				if (!function.hasSubFunction(map)) {
					...
					menuString.append("<li state=\"closed\" iconCls=\""
							+ TSIcon.allTSIcons.get(function.getTSIcon().getId()).getIconClas() + "\" ><span>"
	...
	private static String getLeafOfTree(TSFunction function) {
		...
		//if (function.getFunctionUrl().indexOf("http:") == -1) {
			// update-begin--Author:jg_renjie Date:20150715
			// for：解决rest风格时，传入id错误的问题
			if (function.getFunctionUrl().indexOf("?") == -1) {
				menuString.append("?clickFunctionId=");
			} else {
				menuString.append("&clickFunctionId=");
			}
			// update-end--Author:jg_renjie Date:20150715
			// for：解决rest风格时，传入id错误的问题
			menuString.append(function.getId());
		}
	...
	public static String getBootstrapMenu(Map<Integer, List<TSFunction>> map) {
		...
		for (TSFunction pFunction : pFunctions) {
			...
			menuString.append("			<span class=\"bootstrap-icon\" style=\"background-image: url('"
					+ TSIcon.allTSIcons.get(pFunction.getTSIcon().getId()).getIconPath() + "')\"></span> "
	...
	public static String getWebosMenu(Map<Integer, List<TSFunction>> map) {
		...
		for (TSFunction pFunction : pFunctions) {
			...
			menuString.append("{\"id\":\"" + pFunction.getId() + "\",\"name\":\"" + pFunction.getFunctionName()
					+ "\",\"path\":\"" + TSIcon.allTSIcons.get(pFunction.getTSIcon().getId()).getIconPath()
	 ...
	 * @return
	 */
	public static String getSalvationMultistageMenu(Map<Integer, List<TSFunction>> map, String style) {
		if (map == null || map.size() == 0 || !map.containsKey(0)) {
			return "不具有任何权限,\n请找管理员分配权限";
		}
		StringBuffer menuString = new StringBuffer();
		...
		for (TSFunction function : list) {
			...
			if (!function.hasSubFunction(map)) {
				menuString.append(getChild2(function, 1, map));
				menuString.append("</div>");
			} else {
				//menuString.append("<ul  class=\"nav nav_lf nav-second-level\" >");
				menuString.append(getChild2(function, 1, map));
				menuString.append("</div>");
			}
			curIndex++;
	...
	public static String getChild2(TSFunction parent, int level, Map<Integer, List<TSFunction>> map) {
		StringBuffer menuString = new StringBuffer();
		List<TSFunction> list = map.get(level);
		//判断这些二级菜单是否有三级子菜单
		boolean existsThirdLevelMenu = false;
		for(TSFunction function : list){
			if (function.getTSFunction().getId().equals(parent.getId())) {
				if (function.hasSubFunction(map)) {
					existsThirdLevelMenu = true;
				}
			}
		}
		for (TSFunction function : list) {
			...
			if (function.getTSFunction().getId().equals(parent.getId())) {
				if (!function.hasSubFunction(map)) {
					if(existsThirdLevelMenu){
						menuString.append("<div   title='" + getMutiLang(function.getFunctionName()) + "'>");
					}
					menuString.append(getLeaf3(function));
					if(existsThirdLevelMenu) {
						menuString.append("</div>");
					}
				} else if (map.containsKey(level + 1)) {
					menuString.append("<div   title='" + getMutiLang(function.getFunctionName()) + "'>");
					menuString.append(getChild3(function, level + 1, map));
					menuString.append("</div>");
	
				}
			}
			//menuString.append("<div style='clear:both'></div></div>");
		}
		return menuString.toString();
	}
	
	private static String getChild3(TSFunction parent, int level, Map<Integer, List<TSFunction>> map) {
		StringBuffer menuString = new StringBuffer();
		List<TSFunction> list = map.get(level);
		for (TSFunction function : list) {
			if (function.getTSFunction().getId().equals(parent.getId())) {
				if (!function.hasSubFunction(map)) {
					...
					menuString.append("<div   class='dis_margin'>");
					//menuString.append("<div></div>");// easy ui
														// 默认展开第一级,所以这里设置一个控制,就不展开了
					menuString.append("<span class='jia'><a href='javascript:void(0);' class='imgdis'>" + getMutiLang(function.getFunctionName()) + "</a></span>");
					menuString.append("<ul>");
					menuString.append(getChild4(function, level + 1, map));
					menuString.append("</ul></div>");
	...
	private static String getChild4(TSFunction parent, int level, Map<Integer, List<TSFunction>> map) {
		StringBuffer menuString = new StringBuffer();
		List<TSFunction> list = map.get(level);
		for (TSFunction function : list) {
			if (function.getTSFunction().getId().equals(parent.getId())) {
				if (!function.hasSubFunction(map)) {
					menuString.append(getLeaf4(function));
				} else if (map.containsKey(level + 1)) {
					menuString.append("<div   class='dis_margin'>");
					menuString.append("<span class='jia'><a href='javascript:void(0);' class='imgdis'>" + getMutiLang(function.getFunctionName()) + "</a></span>");
					menuString.append("<ul>");
	...
	private static String getLeaf3(TSFunction function) {
		StringBuffer menuString = new StringBuffer();
//		String icon = "folder";
//		if (function.getTSIcon() != null) {
//			icon = TSIcon.allTSIcons.get(function.getTSIcon().getId()).getIconClas();
//		}
		menuString.append("<p>");
		menuString.append("<a");
		menuString.append(" href='javascript:void(0);' src='");
		menuString.append(function.getFunctionUrl());
		menuString.append("&clickFunctionId=");
		menuString.append(function.getId());
		menuString.append("'  class='cs-navi-tab'><em>");
		menuString.append(getMutiLang(function.getFunctionName()));
		menuString.append("</em></a></p>");
		return menuString.toString();
	}
	
	private static String getLeaf4(TSFunction function) {
		StringBuffer menuString = new StringBuffer();
		String icon = "folder";
		if (function.getTSIcon() != null) {
			icon = TSIcon.allTSIcons.get(function.getTSIcon().getId()).getIconClas();
		...
		menuString.append("<li>");
		menuString.append("<a class='cs-navi-tab");
		menuString.append("' href='javascript:void(0);' src='");
		menuString.append(function.getFunctionUrl());
		menuString.append("&clickFunctionId=");
		menuString.append(function.getId());
		menuString.append("'> <em>");
		menuString.append(getMutiLang(function.getFunctionName()));
		menuString.append("</em></a></li>");
		return menuString.toString();
	}
		...
		for (TSFunction function : list) {
			...
			if (!function.hasSubFunction(map)) {
				...
				menuString.append("<a href=\"#\" class=\"dropdown-toggle\" ><i class=\""
						+ SysFAIconEnum.toEnum(function.getTSIcon().getIconClas()).getThemes() + "\"></i>");
	...
	public static String getHplusMultistageTree(Map<Integer, List<TSFunction>> map) {
		...
		for (TSFunction function : list) {
			if (!function.hasSubFunction(map)) {
				menuString.append(getLeafOfHplusTree1(function));
			} else {
				menuString.append("<li>");
				menuString.append("<a href=\"#\" class=\"\" ><i class=\""
						+ SysFAIconEnum.toEnum(function.getTSIcon().getIconClas()).getThemes() + "\"></i>");
				menuString.append("<span class=\"nav-label\">");
				menuString.append(getMutiLang(function.getFunctionName()));
				menuString.append("</span>");
				menuString.append("<span class=\"fa arrow\">");
				menuString.append("</span>");
				menuString.append("</a><ul  class=\"nav nav_lf nav-second-level\" >");
	...
	private static String getHplusSubMenu(TSFunction parent, int level, Map<Integer, List<TSFunction>> map) {
		String ulClassStyle = "nav nav_lf nav-second-level";
		if(level == 1){
			ulClassStyle = "nav nav_lf nav-third-level";
		}
		StringBuffer menuString = new StringBuffer();
		List<TSFunction> list = map.get(level);
		...
		for (TSFunction function : list) {
			if (function.getTSFunction().getId().equals(parent.getId())) {
				if (!function.hasSubFunction(map)) {
					...
					menuString.append("<span class=\"fa arrow\">");
					menuString.append("</span>");
					menuString.append("</a><ul  class=\"" + ulClassStyle + "\" >");
	 ...
	 * @return
	 */
	private static String getLeafOfHplusTree1(TSFunction function) {
		StringBuffer menuString = new StringBuffer();
		String icon = "folder";
		if (function.getTSIcon() != null) {
			icon = TSIcon.allTSIcons.get(function.getTSIcon().getId()).getIconClas();
		...
		if (url != null && function.getFunctionUrl().indexOf("http:") == -1) {
			if (function.getFunctionUrl().indexOf("?") == -1) {
				url = url + "?clickFunctionId=" + function.getId();
			} else {
				url = url + "&clickFunctionId=" + function.getId();
			}
		}
		
		menuString.append("<li> <a class=\"J_menuItem\" href=\"").append(url).append("\">");
		menuString.append(
				"<i class=\"" + SysFAIconEnum.toEnum(function.getTSIcon().getIconClas()).getThemes() + "\"></i>");
		menuString.append(name);
		menuString.append("</a></li>");
		return menuString.toString();
	}
