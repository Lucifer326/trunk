...
import org.jeecgframework.tag.vo.hplus.ColumnValue;
import org.jeecgframework.tag.vo.hplus.DataGridColumn;
import org.jeecgframework.tag.vo.hplus.DataGridUrl;
import org.jeecgframework.tag.vo.hplus.OptTypeDirection;
...
public class DataGridTag extends TagSupport {
	...
	private boolean showCheckbox = false;// 隐藏复选框
	// private boolean frozenColumn=false; // 是否是冰冻列 默认不是
	private String langArg;
	
	// update-start--Author:zhoujf Date:20150608 for：修改增加easyui
	// css主题目录切换的属性，默认default兼容前版本
	protected String cssTheme ;
	
	private boolean showToggle = false;//是否启用dagatrid右上角的切换按钮
	
	private boolean sortable = false;//是否开启datagrid的表头排序
	 ...
	 * @param width2
	 */
	public void setToolbar(String url, String title, String icon, String exp,String onclick, String funname,String operationCode, String width2, String height2,String styleClass) {
		DataGridUrl dataGridUrl = new DataGridUrl();
		dataGridUrl.setTitle(title);
		dataGridUrl.setUrl(url);
		...
		dataGridUrl.setStyleClass(styleClass);
	...
	public void setColumn(String title, String field, Integer width, String rowspan, 
			...
			String formatter,String formatterjs, boolean hidden, boolean selectHidden,String replace, 
			...
			String style,String downloadName,boolean isAuto,String extendParams,String editor,String dictCondition) {
		...
		dataGridColumn.setSelectHidden(selectHidden);//2016-3-27 LandyChang
		...
		// update-begin--Author:anchao Date:20140826
		// for：[bugfree号]数据列权限控制--------------------
		...
		// update-end--Author:anchao Date:20140826
		// for：[bugfree号]数据列权限控制--------------------
		...
		if (!StringUtils.isBlank(dictionary)&&(!popup)) {
			if(dictionary.contains(",")){
				...
				String sql = "select " + dic[1] + " as field," + dic[2]
						+ " as text from " + dic[0];
				//增加查询条件
				if(!StringUtils.isBlank(dictCondition)){
					sql += dictCondition;
				}
				systemService = ApplicationContextUtil.getContext().getBean(
						SystemService.class);
...
//			SysThemesEnum sysThemesEnum = SysThemesUtil.getSysTheme((HttpServletRequest) super.pageContext.getRequest());
			//if (style.equals("easyui")||treegrid) {
...
//				}
			...
			// ace界面下table的输出
			//}else if(style.equals("hplus_blue")){
			// -----增加hplus处理方式
			// author:gjq-----------date：20161218--------------------------
				out.print(bootstrapdatatables1().toString());
				out.flush();
			//}else{
	...
	/**
	 * bootstrap table 构造方法 by gjq
	 * 
	 * @return
	 */
	public StringBuffer bootstrapdatatables1() {
		String grid = "";
		...
		// sb.append("queryParams:function(params){return{limit:params.limit,offset: params.offset, pageindex:params.pageNumber}},");

		// 修复分页bug-start 1.0
		sb.append("queryParams:function(params){var queryParams=new Object();$('#" + name
				+ "searchlist').find('*').each(function(){queryParams[$(this).attr('name')]=$(this).val();});jQuery.extend(params, queryParams);return params}, \n");
		// 修复分页bug-end 1.0

		sb.append("columns:[[");
		this.getField(sb);
		sb.append("]],\n");
		this.getFrozenIndex(sb);
		if (StringUtil.isNotEmpty(onDblClick)) {
			sb.append("onDblClickRow:function(rowData){" + onDblClick + "(rowData);},\n");
		}
		...
		// 修复分页bug-start 1.0
		sb.append("var queryParams=new Object();\n");
		sb.append("$(\'#" + name
				+ "searchlist\').find('*').each(function(){queryParams[$(this).attr('name')]=$(this).val();});\n");
		sb.append("$(\'#" + name + "\').bootstrapTable('refresh',{url:'" + actionUrl + "&field=" + fields
				+ "',query:queryParams,pageNumber:1});\n");
		// sb.append(" $(\'#" + name + "\').bootstrapTable(\'refresh\');\n");//
		// 修复分页bug-end 1.0

		sb.append("}catch(ex){}\n");
		sb.append("}\n");
		sb.append("function reload" + name + "(){" + "$(\'#" + name + "\').bootstrapTable(\'refresh\');" + "}\n");
		sb.append("function get" + name + "Selected(field){return $(\'#" + name
				+ "\').bootstrapTable(\'getSelections\');}\n");
		sb.append("function getSelected(field){" + "var row = $(\'#\'+gridname).bootstrapTable(\'getSelections\');"
				+ "if(row!=null)" + "{" + "value= row[0][field];" + "}" + "else" + "{" + "value=\'\';" + "}"
				+ "return value;" + "}\n");
		sb.append("function get" + name + "Selections(field){" + "var ids = [];" + "var rows = $(\'#" + name
				+ "\').bootstrapTable(\'getSelections\');" + "for(var i=0;i<rows.length;i++){"
				+ "ids.push(rows[i][field]);" + "}" + "ids.join(\',\');" + "return ids" + "};\n");
		sb.append("function getSelectRows(){\n");
		sb.append("	return $(\'#" + name + "\').bootstrapTable('getSelections');\n");
		sb.append("}\n");
		// update-begin longjb 20150515 for:新增表头定义存储和恢复函数
		sb.append(" function saveHeader(){\n");
		sb.append(
				" var hiddenColumnsFields=null;var visibleColumnsFields=null;;var easyextends=false;try{columnsFields = $('#"
						+ name + "').bootstrapTable('getColumns');hiddenColumnsFields=$('#" + name
						+ "').bootstrapTable('getHiddenColumns');visibleColumnsFields=$('#" + name
						+ "').bootstrapTable('getVisibleColumns');easyextends=true;\n");
		sb.append("}catch(e){hiddenColumnsFields=$('#" + name
				+ "').bootstrapTable('getHiddenColumns');visibleColumnsFields=$('#" + name
				+ "').bootstrapTable('getVisibleColumns');}\n");
		sb.append("	var cols = storage.get( '" + name + "hiddenColumns');var init=true;	if(cols){init =false;} "
				+ "var hiddencolumns = [];"
				+ "for(var i=0;i< hiddenColumnsFields.length;i++) {if(easyextends){hiddencolumns.push({field:hiddenColumnsFields[i].field,hidden:true,visible:false});}}"
				+ "for(var i=0;i< visibleColumnsFields.length;i++) {if(easyextends){hiddencolumns.push({field:visibleColumnsFields[i].field,hidden:false,visible:true});}}\n");
		...
		sb.append("if(cols.visible!=false)$('#" + name
				+ "').bootstrapTable((cols[i].hidden==true?'hideColumn':'showColumn'),cols[i].field);\n");
		sb.append("}catch(e){\n");
		sb.append("}\n");
		sb.append("}\n");
		sb.append("}\n");
		...
		sb.append("	try{\n");
		sb.append("  $('#" + name
				+ "').bootstrapTable((cols.visible==false?'hideColumn':'showColumn'),cols[i].field);\n");
		sb.append("}catch(e){\n");
		sb.append("}\n");
		sb.append("}\n");
		sb.append("}\n");
		...
		if (columnList.size() > 0) {
			...
			sb.append("$(\'#" + name
					+ "searchlist\').find('*').each(function(){queryParams[$(this).attr('name')]=$(this).val();});\n");
			//sb.append("$(\'#" + name + "\').bootstrapTable('refresh',{query:queryParams,pageNumber:1}); }}\n");
			sb.append("$(\'#" + name + "\').bootstrapTable('refresh',{url:\'" + actionUrl + "&field=" + fields
					+ "',query:queryParams,pageNumber:1}); }}\n");
			// sb.append("$(\'#" + name + "\').bootstrapTable({url:'" +actionUrl + "&field=" + searchFields + "',pageNumber:1});" +
			// "}}\n");
			// 修复分页bug-end 1.0
			...
			sb.append("$(\'#" + name + "\').bootstrapTable({url:'" + actionUrl+"&field=" + searchFields
			...
			sb.append(" $(\"#"+name+"searchlist\").find(\":input\").val(\"\");\n");
			...
			// sb.append(func);
//			sb.append("var queryParams=$(\'#" + name + "\').bootstrapTable('getOptions','queryParams');\n");
//			sb.append("$(\'#" + name
//					+ "searchlist\').find('*').each(function(){queryParams[$(this).attr('name')]=$(this).val();});\n");
//			sb.append("$(\'#" + name + "\').refresh({url:'" + actionUrl+"&field=" + searchFields
//					+ "',pageNumber:1});\n");
			// update by jg_renjie at 2016/1/11 for:TASK #823
			// 增加form实现Form表单验证,此处避免reset时走验证，代码做了冗余
			sb.append("}\n");
		...
		if ("group".equals(getQueryMode())) {
			...
			sb.append("<h5>搜索条件</h5>\n");
			sb.append("<div class=\"ibox-tools\">\n");
			sb.append("<a class=\"collapse-link\">\n");
			sb.append("<i class=\"fa fa-chevron-up\"></i>\n");
			sb.append("</a>\n");
			sb.append("</div>\n");
			sb.append("</div>\n");
			// 检索内容区域
			sb.append("<div class=\"ibox-content whj-padding\">\n");
			...
			sb.append("<ul class=\"ulist\" id=\"" + name + "searchlist\">\n");
			for (DataGridColumn col : columnList) {
				if (col.isQuery()) {
					sb.append("<li>\n");
					sb.append("<span class=\"fl labeltext\">\n");
					...
					if ("single".equals(col.getQueryMode())) {
						if (!StringUtil.isEmpty(col.getReplace())) {
							sb.append("<select name=\"" + col.getField().replaceAll("_", "\\.")
									+ "\"   class=\"form-control w-196\"> \n");
							sb.append(StringUtil.replaceAll("<option value =\"\" >{0}</option>\n", "{0}",
							...
							if (col.getDictionary().contains(",") && col.isPopup()) {
								...
								//增加查询条件
								if(!StringUtils.isBlank(col.getDictCondition())){
								...
								// />
								sb.append("<input type=\"text\"  id=\"" + col.getField().replaceAll("_", "\\.")
										+ "\" name=\"" + col.getField().replaceAll("_", "\\.")
										+ "\" style=\"width: 50%\"  class=\"form-control fl\" value=\"\" /> \n");
								sb.append("<button class=\"btn btn-primary btnh\" id=\"btnout"
										+ col.getField().replaceAll("_", "\\.")
										+ "\" style=\"width: 24%;margin-bottom:0;\"  onClick=\"inputClick($('#"
										+ col.getField().replaceAll("_", "\\.") + "'), '" + dic[1] + "','" + dic[0]
										+ "');\" type=\"button\"><i class=\"fa fa-search\"></i>&nbsp;选择</button>\n");
								sb.append(
										"<button class=\"btn btn-primary btnh\"  style=\"width: 24%;margin-bottom:0;\"  onClick=\"javascript:$(\'#"
												+ col.getField().replaceAll("_", "\\.")
												+ "\').val(\'\')\" type=\"button\"><i class=\"fa \"></i>&nbsp;清空</button>\n");
							...
							sb.append(
									"<input onkeypress=\"EnterPress(event)\" onkeydown=\"EnterPress()\"  type=\"text\" id=\""
											+ col.getField() + "\" name=\"" + col.getField().replaceAll("_", "\\.")
											+ "\"  " + extendAttribute(col.getExtend())
											+ "  class=\"form-control\"  />\n");
						}
						...
						/*
						 * sb.append("<li>\n"); sb.append(
						 * "<span class=\"fl labeltext\">\n");
						 * sb.append(col.getTitle()+":");
						 * sb.append("</span>\n"); sb.append(
						 * "<div class=\"fl\">\n");
						 */
						sb.append("<input type=\"text\" id=\"" + col.getField() + "_begin\" name=\"" + col.getField()
								+ "_begin\"   class=\"laydate-icon form-control layer-date fl w-148\"  "
								+ extendAttribute(col.getExtend()) + " />\n");
						sb.append("<span class=\"fl communicate\">―</span>\n");
						sb.append("<input type=\"text\" id=\"" + col.getField() + "_end\" name=\"" + col.getField()
								+ "_end\"   " + extendAttribute(col.getExtend())
								+ "  class=\"laydate-icon form-control layer-date fl w-148\"/>\n");
						sb.append("</div>\n");
						sb.append("<div class=\"clear\"></div>\n");
						sb.append("</li>\n");
						// update-end--Author:longjb Date:20150323
						// for：增加class属性便于样式控制
					}
			...
			sb.append("<li id=\"div_extend\">\n");
			sb.append("<div class=\"clear\"></div>\n");
			sb.append("</li>\n");
			sb.append("<li id=\"div_extend2\">\n");
			sb.append("<div class=\"clear\"></div>\n");
			sb.append("</li>\n");
			sb.append("<li>\n");
			sb.append("<button class=\"btn btn-primary btnh\" id=\"btnSearch\" type=\"button\" onclick=\"" + name
					+ "search()\"><i class=\"fa fa-search\"></i>"
					+ MutiLangUtil.getMutiLangInstance().getLang("common.query") + "</button>");
			...
			sb.append("<li>\n");
			sb.append("<button class=\"btn btn-primary btnh\" id=\"btnSearch\" type=\"button\" onclick=\"" + name 
					+ "searchReset('" + name + "')\"><i class=\"fa fa-empty\"></i>"
					+ MutiLangUtil.getMutiLangInstance().getLang("common.reset") + "</button>");
			sb.append("</li>\n");
			/*********************************/
			sb.append("</ul>\n");
			sb.append("</div>\n");
		...
		if (toolBarList.size() > 0) {
			sb.append("<div class=\"btn-fl\">\n");
			for (DataGridUrl toolBar : toolBarList) {
				sb.append("<button type=\"button\" class=\"btn btn-outline " + toolBar.getStyleClass() + " \" ");
				...
				sb.append(">" + toolBar.getTitle() + "</button>\n");
			}
			sb.append("</div>\n");
		}
		// 工具按钮区结束
	...
	public StringBuffer bootstrapdatatables() {
		...
		sb.append("}\n");
		sb.append("function reload" + name + "(){" + "$(\'#" + name + "\').bootstrapTable(\'refresh\');" + "}\n");
		sb.append("function get" + name + "Selected(field){return $(\'#" + name + "\').bootstrapTable(\'getSelections\');}\n");
		sb.append("function getSelected(field){" + "var row = $(\'#\'+gridname).bootstrapTable(\'getSelections\');" + "if(row!=null)" + "{" + "value= row[0][field];" + "}" + "else" + "{" + "value=\'\';" + "}" + "return value;" + "}\n");
		sb.append("function get" + name + "Selections(field){" + "var ids = [];" + "var rows = $(\'#" + name + "\').bootstrapTable(\'getSelections\');" + "for(var i=0;i<rows.length;i++){" + "ids.push(rows[i][field]);" + "}" + "ids.join(\',\');" + "return ids" + "};\n");
		sb.append("function getSelectRows(){\n");
		sb.append("	return $(\'#"+name+"\').bootstrapTable('getSelections');\n");
		sb.append("}\n");
		// update-begin longjb 20150515 for:新增表头定义存储和恢复函数
		sb.append(" function saveHeader(){\n");
		sb.append(" var hiddenColumnsFields=null;var visibleColumnsFields=null;;var easyextends=false;try{columnsFields = $('#"+name+"').bootstrapTable('getColumns');hiddenColumnsFields=$('#"+name+"').bootstrapTable('getHiddenColumns');visibleColumnsFields=$('#"+name+"').bootstrapTable('getVisibleColumns');easyextends=true;\n");
		sb.append("}catch(e){hiddenColumnsFields=$('#"+name+"').bootstrapTable('getHiddenColumns');visibleColumnsFields=$('#"+name+"').bootstrapTable('getVisibleColumns');}\n");
		sb.append("	var cols = storage.get( '"+name+"hiddenColumns');var init=true;	if(cols){init =false;} " +
				"var hiddencolumns = [];"
				+ "for(var i=0;i< hiddenColumnsFields.length;i++) {if(easyextends){hiddencolumns.push({field:hiddenColumnsFields[i].field,hidden:true,visible:false});}}"
				+ "for(var i=0;i< visibleColumnsFields.length;i++) {if(easyextends){hiddencolumns.push({field:visibleColumnsFields[i].field,hidden:false,visible:true});}}\n");
		...
		sb.append( "for(var i=0;i<cols.length;i++){\n");
		sb.append( "	try{\n");
		sb.append("if(cols.visible!=false)$('#"+name+"').bootstrapTable((cols[i].hidden==true?'hideColumn':'showColumn'),cols[i].field);\n");
		sb.append( "}catch(e){\n");
		sb.append( "}\n");
		sb.append( "}\n");
		sb.append( "}\n");
		...
		sb.append( "for(var i=0;i<cols.length;i++){\n");
		sb.append( "	try{\n");
		sb.append("  $('#"+name+"').bootstrapTable((cols.visible==false?'hideColumn':'showColumn'),cols[i].field);\n");
		sb.append( "}catch(e){\n");
		sb.append( "}\n");
		sb.append( "}\n");
		sb.append( "}\n");
		...
		if (columnList.size() > 0) {
			...
			sb.append("if($(\"#"+name+"Form\").Validform({tiptype:3}).check()){\n");
			// update by jg_renjie at 2016/1/11 for:TASK #823 增加form实现Form表单验证
			sb.append("var queryParams=new Array();\n");
			sb.append("$(\'#" + name + "searchlist\').find('*').each(function(){queryParams[$(this).attr('name')]=$(this).val();});\n");
			sb.append("$(\'#" + name + "\').bootstrapTable('refresh',{query:queryParams,pageNumber:1}); }}\n");
			//sb.append("$(\'#" + name + "\').bootstrapTable({url:'" + actionUrl + "&field=" + searchFields + "',pageNumber:1});" + "}}\n");
			...
			sb.append("function dosearch(params){\n");
			sb.append("var jsonparams=$.parseJSON(params);\n");
			sb.append("$(\'#" + name + "\').bootstrapTable({url:'" + actionUrl + "&field=" + searchFields + "',queryParams:jsonparams});" + "}\n");
...
//			  searchboxFun(sb,grid);
			...
			sb.append("var queryParams=$(\'#" + name + "\').bootstrapTable('getOptions','queryParams');\n");
			sb.append("$(\'#" + name + "searchlist\').find('*').each(function(){queryParams[$(this).attr('name')]=$(this).val();});\n");
			sb.append("$(\'#" + name + "\').refresh({url:'" + actionUrl + "&field=" + searchFields + "',pageNumber:1});\n");
			// update by jg_renjie at 2016/1/11 for:TASK #823
			// 增加form实现Form表单验证,此处避免reset时走验证，代码做了冗余
			sb.append("}\n");
			//this.setPager(sb, grid);
		...
		if("group".equals(getQueryMode())){
			...
			for(DataGridColumn col: columnList){
				if(col.isQuery()){
					...
					if("single".equals(col.getQueryMode())){
						if(!StringUtil.isEmpty(col.getReplace())){
							sb.append("<select name=\""+col.getField().replaceAll("_","\\.")+"\"   class=\"form-control w-196\"> \n");
							sb.append(StringUtil.replaceAll("<option value =\"\" >{0}</option>\n", "{0}", MutiLangUtil.getMutiLangInstance().getLang("common.please.select")));
							String[] test = col.getReplace().split(",");
							String text = "";
							String value = "";						
							for (String string : test) {
							...
							//	<input type="text" name="order_code"  style="width: 100px"  class="searchbox-inputtext" value="" onClick="inputClick(this,'account','user_msg');" />
								sb.append("<input type=\"text\"  id=\""+col.getField().replaceAll("_","\\.")+"\" name=\""+col.getField().replaceAll("_","\\.")+"\" style=\"width: 50%\"  class=\"form-control fl\" value=\"\" /> \n");
								sb.append("<button class=\"btn btn-primary btnh\" id=\"btnout"
										+ col.getField().replaceAll("_", "\\.")
										+ "\" style=\"width: 24%;margin-bottom:0;\"  onClick=\"inputClick($('#"
										+ col.getField().replaceAll("_", "\\.") + "'), '" + dic[1] + "','" + dic[0]
						...
						sb.append(getAutoSpan(col.getField().replaceAll("_","\\."),extendAttribute(col.getExtend())));
							...
							sb.append("<input onkeypress=\"EnterPress(event)\" onkeydown=\"EnterPress()\"  type=\"text\" id=\""+col.getField()+"\" name=\""+col.getField().replaceAll("_","\\.")+"\"  "+extendAttribute(col.getExtend())+"  class=\"form-control\"  />\n");
						}
				...
				/*	sb.append("<li>\n");
					...
					sb.append("</span>\n");
					sb.append("<div class=\"fl\">\n");*/
					sb.append("<input type=\"text\" id=\""+col.getField()+"_begin\" name=\""+col.getField()+"_begin\"   class=\"laydate-icon form-control layer-date fl w-148\"  "+extendAttribute(col.getExtend())+" />\n");
						sb.append("<span class=\"fl communicate\">―</span>\n");
					sb.append("<input type=\"text\" id=\""+col.getField()+"_end\" name=\""+col.getField()+"_end\"   "+extendAttribute(col.getExtend())+"  class=\"laydate-icon form-control layer-date fl w-148\"/>\n");
					sb.append("</div>\n");
					sb.append("<div class=\"clear\"></div>\n");
					sb.append("</li>\n");
						// update-end--Author:longjb Date:20150323
			...
			sb.append("</li>\n");
			sb.append("<li>\n");
			sb.append("<button class=\"btn btn-primary btnh\" id=\"btnSearch\" type=\"button\" onclick=\""+  name+ "search()\"><i class=\"fa fa-search\"></i>"+MutiLangUtil.getMutiLangInstance().getLang("common.query")+"</button>");
			sb.append("</li>\n");
	...
	public StringBuffer end() {
		...
		sb.append("function reloadTable(){");
		sb.append("try{");
		sb.append("console.log(gridname);");
		sb.append("alert(gridname+\'inhplus\');");
					...
					if (col.isQuery()) {
						...
						if("single".equals(col.getQueryMode())){
							if(!StringUtil.isEmpty(col.getReplace())){
								...
								// update-start--Author:longjb Date:20150323
								// for：增加class属性便于样式控制
	...
	protected void getOptUrl(StringBuffer sb) {
		...
		for (DataGridUrl dataGridUrl : list) {
			...
			// update-start--Author: jg_huangxg Date:20151204
			// for：增加styke属性便于样式控制
	...
	protected void getFrozenIndex(StringBuffer sb){
		int i=0,j=0;
		...
		if(j>0){
			sb.append("fixedColumns:true,fixedNumber:"+j+",");
		}
	...
	protected void getField(StringBuffer sb,int frozen) {
		//复选框
		if (showCheckbox&&frozen==1) {
			sb.append("{field:\'checkboxStatus\',checkbox:\'true\'");
			sb.append(",formatter:function (value, row, index){");
			sb.append("var checkboxStatus;");
			sb.append("if(row.checkboxStatus=='1'){");
			sb.append("checkboxStatus=true;");
			sb.append("}else{");
			sb.append("checkboxStatus=false;}");
			sb.append("if(checkboxStatus == true)");
			sb.append("return {");
			sb.append("disabled: false,");
			sb.append("checked: true");
			sb.append("};");
			sb.append("return checkboxStatus;");
			sb.append("}},");
		}
		int i = 0;
		for (DataGridColumn column : columnList) {
			...
			if(field.equals("checkboxStatus")){
				continue;
			}
			...
			if(StringUtils.isNotEmpty(column.getExtendParams())){
				...
				// update-start-Author:zhangguoming Date:20140921 for：TASK #458
				// 列表hidden=false，才是隐藏好像有点问题
				// 隐藏字段
			if (column.isHidden()) {
				sb.append(",visible:false");
			}
			
			//2016-3-27 LandyChang 
			if (!column.isSelectHidden()) {
				sb.append(",switchable:false");
			}
			
			
				// update-end-Author:zhangguoming Date:20140921 for：TASK #458
				// 列表hidden=false，才是隐藏好像有点问题
			if (!treegrid) {
					// 字段排序
