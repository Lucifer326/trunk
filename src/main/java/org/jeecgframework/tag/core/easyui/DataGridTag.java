	...
	public void setColumn(String title, String field, Integer width, String rowspan, 
		...
		if (!StringUtils.isBlank(dictionary)&&(!popup)) {
			if(dictionary.contains(",")){
				...
				//修改后
				//字典关联区划, 通过用户登录后的区划ID 和 字典类型编码来查询可以使用的字典条目数
				...
				typeService = ApplicationContextUtil.getContext().getBean(
														TypeServiceI.class);
				//根据字典类型编码 和 区划  查出 所有 条目			
				List<TSType> typeList = typeService.getItemsByTypegroupcodeAndRegionId(dictionary, regionId);
				//
...
//-----author:jg_longjb----end-----date:20150408--------for:读取cookie主题样式 ace界面下table的输出 
	...
	public StringBuffer end1() {
		...
		sb.append("	return $(\'#"+name+"\')."+grid+"('getChecked');");
		sb.append("}");
		//update-begin longjb 20150515 for:新增表头定义存储和恢复函数
		sb.append(" function saveHeader(){");
		sb.append(" var columnsFields =null;var easyextends=false;try{columnsFields = $('#"+name+"')."+grid+"('getColumns');easyextends=true;");
		sb.append("}catch(e){columnsFields =$('#"+name+"')."+grid+"('getColumnFields');}");
		...
		sb.append( "	try{");
		sb.append("if(cols.visible!=false)$('#"+name+"')."+grid+"((cols[i].hidden==true?'hideColumn':'showColumn'),cols[i].field);");
		sb.append( "}catch(e){");
		sb.append( "}");
		sb.append( "}");
		sb.append( "}");
		...
		sb.append( "for(var i=0;i<cols.length;i++){");
		sb.append( "	try{");
		sb.append("  $('#"+name+"')."+grid+"((cols.visible==false?'hideColumn':'showColumn'),cols[i].field);");
		sb.append( "}catch(e){");
		sb.append( "}");
		sb.append( "}");
		sb.append( "}");
		...
		if(hasQueryColum(columnList)){
			...
			//如果表单是组合查询
			sb.append("<div class=\"fl searchdiv\" style=\"margin-right:10px;\">");
							...
							if(!StringUtil.isEmpty(col.getReplace())){
								...
								if(col.getDictionary().contains(",")&&col.isPopup()){
									...
									String sql = "select " + dic[1] + " as field," + dic[2]
											+ " as text from " + dic[0];
									//增加查询条件
									if(!StringUtils.isBlank(col.getDictionary())){
										sql += col.getDictionary();
									}
...
//									List<TSType> types = typedatas.get(col.getDictionary().toLowerCase());
									...
									typeService = ApplicationContextUtil.getContext().getBean(
																			TypeServiceI.class);
									//根据字典类型编码 和 区划  查出 所有 条目			
									List<TSType> types = typeService.getItemsByTypegroupcodeAndRegionId(col.getDictionary(), regionId);
									//
									sb.append("<select name=\""+col.getField().replaceAll("_","\\.")+"\" WIDTH=\"100\" style=\"width: 104px\"> ");
									sb.append(StringUtil.replaceAll("<option value =\"\" >{0}</option>", "{0}", MutiLangUtil.getMutiLangInstance().getLang("common.please.select")));
									if (types != null) {
			...
			//	update-end--Author:longjb  Date:20150323 for：增加class属性便于样式控制
				...
				sb.append("<div class=\"clear clearfix\"></div>");
				//
			}
			sb.append("</form></div>");
		}
...
//            update-end-Author:zhangguoming  Date:20140921 for：TASK #458 列表hidden=false，才是隐藏好像有点问题
				...
				}else if(column.getFormatter()!=null)
				{
					...
					for (ColumnValue columnValue : columnValueList) {
						if (columnValue.getName().equals(column.getField())) {
							...
							sb.append("var valArray = value.toString().split(',');");
	...
	protected void searchboxFun(StringBuffer sb,String grid)
	{
		...
		sb.append("var queryParams=$(\'#" + name + "\').datagrid('options').queryParams;");
		sb.append("queryParams[name]=value;queryParams.searchfield=name;$(\'#" + name + "\')." + grid + "(\'load\');}");
