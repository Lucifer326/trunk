 ...
 * 类描述：标签工具类
 * 
 * @author: 张代浩 @date： 日期：2012-12-28 时间：上午09:58:00
 * @version 1.1
...
public class TagUtil {
	...
	public static Object fieldNametoValues(String FiledName, Object o) {
		...
		if (value != "" && value != null && (FiledName.indexOf("_") != -1 || FiledName.indexOf(".") != -1)) {
			// update-start--Author:zhangguoming Date:20140827
			// for：功能增强，添加处理对象中List<Object>属性字段的解析
			...
			// update-end--Author:zhangguoming Date:20140827
			// for：功能增强，添加处理对象中List<Object>属性字段的解析
		}
		if (value != "" && value != null) {
			// update-begin--Author:jb_longjb 龙金波 Date:20150313
			// for：处理json中可能包含的特殊字符，防止页面加载出错
	...
	// update-end--Author:jb_longjb 龙金波 Date:20150313
	// for：处理json中可能包含的特殊字符，防止页面加载出错
	...
	private static String listtojson(String[] fields, int total, List<?> list, String[] footers) throws Exception {
		...
		for (int j = 0; j < list.size(); ++j) {
			...
			for (i = 0; i < fields.length; ++i) {
				...
				} else if (list.get(j) instanceof Object[]) {
					values[i] = ((Object[]) list.get(j))[i];
				} else {
					values[i] = fieldNametoValues(fieldName, list.get(j));
				}
				jsonTemp.append(
						"\"" + fieldName + "\"" + ":\"" + String.valueOf(values[i]).replace("\"", "\\\"") + "\"");
	...
	private static JSONObject getJson(DataGrid dg) {
		...
		try {
			if (!StringUtil.isEmpty(dg.getFooter())) {
				...
				jObject = JSONObject
						.parseObject(listtojson(dg.getField().split(","), dg.getTotal(), dg.getResults(), null));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private static JSONObject getJson(DataTableReturn dataTable, String field) {
		...
		try {
			jObject = JSONObject
					.parseObject(datatable(field, dataTable.getiTotalDisplayRecords(), dataTable.getAaData()));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return jObject;
		...
		if (datas != null && datas.size() > 0) {
			...
			for (int i = 0; i < datas.size(); i++) {
				...
				for (int j = 0; j < values.length; j++) {
					sb.append("\"" + fields.get(j) + "\":\"" + (values[j] == null ? "" : values[j]) + "\""
							+ (j == values.length - 1 ? "" : ","));
				}
		...
		if (datas != null && datas.size() > 0) {
			...
			for (int i = 0; i < datas.size(); i++) {
				...
				for (Object value : values.values()) {
					sb.append("\"" + fields.get(j) + "\":\"" + (value == null ? "" : value) + "\""
							+ (j == values.size() - 1 ? "" : ","));
					j++;
				}
