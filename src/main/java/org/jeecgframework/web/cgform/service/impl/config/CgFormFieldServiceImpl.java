...
public class CgFormFieldServiceImpl extends CommonServiceImpl implements CgFormFieldServiceI {
	...
	public Boolean judgeTableIsExit(String tableName) {
		...
		try {
			...
			if ("ORACLE".equals(dataBaseType)) {
				...
				Logger.getLogger(CgFormFieldServiceImpl.class)
						.info("添加达梦数据库支持 ---------- judgeTableIsExit方法中添加达梦数据库判断");
				tableNamePattern = tableName.toUpperCase();
			}
	...
	public List<String[]> getDictFieldByTableName(String tableName) {
		StringBuilder hql = new StringBuilder("");
		hql.append("from CgFormFieldEntity f");
		hql.append(" where f.table.tableName=? and f.dictField is not null and f.isShow='Y' ");
		List<CgFormFieldEntity> list = this.findHql(hql.toString(), tableName);
		List<String[]> listResult = new ArrayList<String[]>();
		if(list.size()>0){
			for(int i=0;i<list.size();i++){
				CgFormFieldEntity entity = list.get(i);
				String[] str = new String[3];
				String content = entity.getContent();
				String dicfield = entity.getDictField();
				String fildName = entity.getFieldName();
				str[0]=content;
				str[1]=dicfield;
				str[2]=fildName;
				listResult.add(str);
		...
		if (list != null && list.size() > 0) {
			for (Map<String, Object> map : list) {
				if (map.get("main_field") != null) {
					sql2.append(" and sub.").append((String) map.get("field_name")).append("=").append("main.")
	...
	public Map<String, Object> getFtlFormConfig(String tableName, String version) {
		...
		if (head.getJformType() == CgAutoListConstant.JFORM_TYPE_MAIN_TALBE) {
			...
			if (StringUtils.isNotEmpty(subTableStr)) {
				...
				for (String subTable : subTables) {
					...
					// --author：luobaoli---------date:20150613--------for:
					// 将表单子表中extend_json属性json样式转为普通html样式
		...
		if (fieldList != null) {
			...
			for (Map<String, Object> map : fieldList) {
				...
				if (!"textarea".equals((String) map.get("show_type"))
						&& !"umeditor".equals((String) map.get("show_type"))) {
					list.add(map);
				} else {
		...
		// --author：luobaoli---------date:20150613--------for:
		// 将表单单表或者主表中extend_json属性json样式转为普通html样式
	...
	/**
	 * @author hyzx
	 * @date : 2018年5月3日 下午1:57:21
	 * @param headId
	 *            待更新表的id 绕过minidao采用hql访问数据持久层接口更新数据库版本和同步状态
	 */
	public void updateSynAndVer(String headId) {
		doUpdateSyn(headId);
	}

	private synchronized void doUpdateSyn(String synId) {
		String version = "";
		String hql = "update CgFormHeadEntity t set t.jformVersion =? where t.id =?";
		String hql2 = "from CgFormHeadEntity t where t.id= ?";
		List<CgFormHeadEntity> list = commonDao.findHql(hql2, synId);
		version = list.get(0).getJformVersion();
		int newVersion = Integer.parseInt(version) + 1;
		String newVersion_ = String.valueOf(newVersion + "");
		commonDao.executeHql(hql, newVersion_, synId);
		String hql3 = "update CgFormHeadEntity t set t.isDbSynch = ? where t.id = ?";
		commonDao.executeHql(hql3, "N", synId);
