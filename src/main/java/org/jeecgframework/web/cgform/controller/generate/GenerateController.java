...
public class GenerateController extends BaseController {
	...
	public void dogenerate(CgFormHeadEntity cgFormHead,GenerateEntity generateEntity,CreateFileProperty createFileProperty,
		...
		if (StringUtil.isNotEmpty(cgFormHead.getId())) {
			cgFormHead = cgFormFieldService.getEntity(
					CgFormHeadEntity.class, cgFormHead.getId());
			Map dictMap = formatDictMap();
			generateEntity.setDictMap(dictMap);
	 ...
	 */
	/**
	 * @param codeParamEntityIn
	 * @param subTableListEntity
	 * @param jspMode
	 * @param request
	 * @param response
	 */
	...
	public void dogenerateOne2Many(CodeParamEntity codeParamEntityIn,GenerateSubListEntity subTableListEntity,String jspMode,HttpServletRequest request,HttpServletResponse response){
		...
		try{
			...
			*/
			//将分开调用的方法改为调用一个,通过传参来区分样式
			CgformCodeGenerateOneToMany.oneToManyCreate(subTabParamIn, codeParamEntityIn,mainG,subsG,jspMode);
	...
	private void getCgformConfig(CgFormHeadEntity cgFormHead,
		...
		for(CgformButtonEntity cb:buttons){
			...
			String cbs_cgbsqlStr[] = null;
			if(cbs!=null){
				if(StringUtil.isNotEmpty(cbs.getCgbSqlStr())){
					cbs_cgbsqlStr = cbs.getCgbSqlStr().replaceAll("(\r\n|\r|\n|\n\r)", "").split(";");
					buttonSqlMap.put(cb.getButtonCode(), cbs_cgbsqlStr);
				}else{
					buttonSqlMap.put(cb.getButtonCode(), new String[]{});
				}
			}else{
				buttonSqlMap.put(cb.getButtonCode(), new String[]{});
			}
		...
		String cbsAdd_cgbsqlStr[] = null;
		if(cbsAdd!=null){
			if(StringUtil.isNotEmpty(cbsAdd.getCgbSqlStr())){
				cbsAdd_cgbsqlStr = cbsAdd.getCgbSqlStr().replaceAll("(\r\n|\r|\n|\n\r)", "").split(";");
				buttonSqlMap.put("add", cbsAdd_cgbsqlStr);
			}else{
				buttonSqlMap.put("add", new String[]{});
			}
		}else{
			buttonSqlMap.put("add", new String[]{});
		}
		...
		String cbsUpdate_cgbsqlStr[] = null;
		if(cbsUpdate!=null){
			if(StringUtil.isNotEmpty(cbsUpdate.getCgbSqlStr())){
				cbsUpdate_cgbsqlStr = cbsUpdate.getCgbSqlStr().replaceAll("(\r\n|\r|\n|\n\r)", "").split(";");
				buttonSqlMap.put("update", cbsUpdate_cgbsqlStr);
			}else{
				buttonSqlMap.put("update", new String[]{});
			}
		}else{
			buttonSqlMap.put("update", new String[]{});
		}
		...
		String cbsDelete_cgbsqlStr[] = null;
		if(cbsDelete!=null){
			if(StringUtil.isNotEmpty(cbsDelete.getCgbSqlStr())){
				cbsDelete_cgbsqlStr = cbsDelete.getCgbSqlStr().replaceAll("(\r\n|\r|\n|\n\r)", "").split(";");
				buttonSqlMap.put("delete", cbsDelete_cgbsqlStr);
			}else{
				buttonSqlMap.put("delete", new String[]{});
			}
		}else{
			buttonSqlMap.put("delete", new String[]{});
		}
	...
	public Object doExpandFileTree(String parentNode){
		...
		try{
			if(StringUtil.isEmpty(parentNode)){
				...
				try {
					parentNode =  new String(parentNode.getBytes("UTF-8"), "UTF-8");
	 ...
	 * @throws Exception 
	 */
	public Map formatDictMap() {
		Map<String,String> map = new HashMap<String,String>();
		List<TSTypegroup> typegroupList = systemService.loadAll(TSTypegroup.class);
		for (TSTypegroup entity : typegroupList){
			String groupcode = entity.getTypegroupcode();
			List<TSType> list = entity.getTSTypes();
			String sumStr = "";
			for(int i=0;i<list.size();i++){
				String name = MutiLangUtil.getMutiLangInstance().getLang(list.get(i).getTypename());
				String code = list.get(i).getTypecode();				
				if(i==0){
					sumStr = "\""+name+"_"+code+"\"";
				}else{
					sumStr = sumStr+","+"\""+name+"_"+code+"\"";;
				}
			}
			map.put(groupcode, sumStr);
		}
		return map;
	}
}
