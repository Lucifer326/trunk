...
public class CgAutoListController extends BaseController{
	...
	public void list(String id, HttpServletRequest request,
		...
		//Online表单在线开发创建表单后，进行功能测试时，提交流程，processkey无法获取到，
		//重新写了一个根据表名获取processkey的方法，用来在表单_功能测试时点击提交流程启动流程
		if(StringUtil.isEmpty(processkey)){
			TPProcess process = cgTableService.getProcesskeyConfigId(id);
		...
		request.setAttribute("config_processkey", processkey);
		
		
		Map<String, Object> configs = configService.queryConfigs(id,jversion,processkey);
		//step.2 获取列表ftl模板路径
