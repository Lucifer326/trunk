...
public class ${entityName}Controller extends BaseController {
	...
	public void datagrid(${entityName}Entity ${entityName?uncap_first},HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		...
		if(StringUtil.isEmpty(${entityName?uncap_first}.getId())){
			cq.isNull("${cgformConfig.cgFormHead.treeParentIdFieldName}");
		}else{
			cq.eq("${cgformConfig.cgFormHead.treeParentIdFieldName}", ${entityName?uncap_first}.getId());
	...
	<#if delFlag==true>
			...
			${entityName?uncap_first}.setIsdeleted("1");
			${entityName?uncap_first}Service.doUpdateSql(${entityName?uncap_first});
		<#else>
	...
	public AjaxJson importExcel(HttpServletRequest request, HttpServletResponse response) {
		...
		for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
			...
			try {
				...
				String name = getTableName(${entityName}Entity.class);				
				List<String[]> list = cgFormFieldService.getDictFieldByTableName(name);								
				int line = params.getHeadRows() + params.getTitleRows();
				for (${entityName}Entity ${entityName?uncap_first} : list${entityName}Entitys) {
					if(list.size()>0){//表中存在数据字典字段
						for(int i=0;i<list.size();i++){
							String[] str = list.get(i);						
							String code = getClassValue(${entityName?uncap_first},str[2]).toString();
							TSType tsType = typeService.getTypenameByTypegroupcodeAndTypecode(str[1], code);
							if(tsType==null){
								j.setMsg("第"+(i+1+line)+"行"+str[0]+"字典条目名称错误");
								return j;
							}
						}
					}
